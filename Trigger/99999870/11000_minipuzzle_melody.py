...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[12000066], state=2) # RareBox 맵 별로 유니크하도록 변경해야 하는 값
...
class EndMelodyQuiz_Success(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[12000066], state=1) # RareBox 맵 별로 유니크하도록 변경해야 하는 값
        set_timer(timerId='2', seconds=60, clearAtZero=True, display=False, arg5=0)

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[12000066], arg2=0):
