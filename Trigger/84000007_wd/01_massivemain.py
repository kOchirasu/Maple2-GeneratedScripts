...
class Wait(state.State):
    def on_enter(self):
        ...
        set_effect(triggerIds=[8002], visible=False) # Fireworks
        ...
        set_interact_object(triggerIds=[10001437], state=0) # 7000ms
        set_interact_object(triggerIds=[10001438], state=2) # 9000ms
        set_interact_object(triggerIds=[10001439], state=2) # 12000ms
        set_interact_object(triggerIds=[10001440], state=2) # 15000ms
...
class EntryDelay(state.State):
    def on_enter(self):
        set_timer(timerId='1', seconds=40) # 테스트 수정 가능 지점
...
class MusicChange(state.State):
    def on_enter(self):
        move_user(mapId=84000007, portalId=1, boxId=9000) # 종료 후 펀타임을 위해 전부 스테이지 밖으로 킥
        add_buff(boxIds=[9000], skillId=99940042, level=1, arg4=False, arg5=False) # 불꽃놀이 스킬셋 제공
...
class GameGuide01(state.State):
    def on_enter(self):
        ...
        play_system_sound_in_box(boxIds=[9000], sound='DJDD_Notice_01') # action name="업적이벤트를발생시킨다" arg1="9001" arg2="trigger" arg3="dailyquest_start"/
        give_guild_exp(boxId=0, type=2) # action name="SetMiniGameAreaForHack" boxID="9001" /
        start_mini_game(boxId=9001, round=5, isShowResultUI=False, gameName='WDdancedancestop')
...
class R01DanceTime(state.State):
    ...
    def on_exit(self):
        set_interact_object(triggerIds=[10001437], state=2) # 7000ms
...
class R01DancePattern01(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001438], state=1) # 9000ms
...
class R01DancePattern02(state.State):
    def on_enter(self):
        play_system_sound_in_box(boxIds=[9001], sound='DDStop_Stage_Ready_01')
        set_interact_object(triggerIds=[10001439], state=1) # 12000ms
...
class R01DancePattern03(state.State):
    def on_enter(self):
        play_system_sound_in_box(boxIds=[9001], sound='DDStop_Stage_Ready_01')
        set_interact_object(triggerIds=[10001440], state=1) # 15000ms
...
class R01DancePattern0401(state.State):
    def on_enter(self):
        play_system_sound_in_box(boxIds=[9001], sound='DDStop_Stage_Ready_01')
        set_interact_object(triggerIds=[10001437], state=1) # 7000ms
...
class R01DancePattern0403(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001437], state=2) # 7000ms
        set_interact_object(triggerIds=[10001438], state=0) # 9000ms
...
class R01DancePattern0602(state.State):
    def on_enter(self):
        ...
        set_sound(triggerId=30000, arg2=True) # Silence
        set_effect(triggerIds=[8000], visible=True) # Scratch
        set_interact_object(triggerIds=[10001439], state=0) # 12000ms
...
class R01End(state.State):
    def on_enter(self):
        write_log(logName='dancedancestop', event='9001', arg3='round_clear', arg4='1')
...
class R02Ready(state.State):
    def on_enter(self):
        ...
        end_mini_game_round(winnerBoxId=9001, expRate=0.02) # action name="업적이벤트를발생시킨다" arg1="9001" arg2="trigger" arg3="ddstop_pass"/
...
class R02End(state.State):
    def on_enter(self):
        write_log(logName='dancedancestop', event='9001', arg3='round_clear', arg4='2')
        set_cinematic_ui(type=0)
        set_cinematic_ui(type=2)
        select_camera(triggerId=901, enable=False)
...
class R03End(state.State):
    def on_enter(self):
        write_log(logName='dancedancestop', event='9001', arg3='round_clear', arg4='3')
        set_cinematic_ui(type=0)
        set_cinematic_ui(type=2)
        select_camera(triggerId=901, enable=False)
...
class R04_GambleGuide01(state.State):
    def on_enter(self):
        ...
        write_log(logName='dancedancestop', event='9001', arg3='system_event', arg4='4', arg5='gamble')
...
class R04_JackpotGuide01(state.State):
    def on_enter(self):
        ...
        play_system_sound_in_box(boxIds=[9000], sound='DJDD_Round_06')
        write_log(logName='dancedancestop', event='9001', arg3='system_event', arg4='4', arg5='jackpot')
...
class R04GambleEnd(state.State):
    def on_enter(self):
        write_log(logName='dancedancestop', event='9001', arg3='round_clear', arg4='4')
        set_cinematic_ui(type=0)
        set_cinematic_ui(type=2)
        select_camera(triggerId=901, enable=False)
...
class Fireworks_Lose(state.State):
    def on_enter(self):
        select_camera_path(pathIds=[902,903], returnView=True)
        set_user_value(triggerId=9, key='Fireworks', value=1) # 센서: Fireworks.xml의 불꽃놀이 연출 시작
...
class GameWrapUp(state.State):
    def on_enter(self):
        end_mini_game_round(winnerBoxId=9001, expRate=0.02) # win
        set_mesh(triggerIds=[8900,8901,8902,8903,8904,8905,8906,8907,8908,8909,8910,8911,8912,8913,8914], visible=False, arg3=400, arg4=0, arg5=0) # Barrier
        set_user_value(triggerId=8110, key='Barrier11', value=10)
        set_user_value(triggerId=8120, key='Barrier12', value=10)
        set_user_value(triggerId=8130, key='Barrier13', value=10)
...
class Fireworks_Win(state.State):
    def on_enter(self):
        select_camera_path(pathIds=[902,903], returnView=True)
        set_user_value(triggerId=9, key='Fireworks', value=2) # 센서: Fireworks.xml의 불꽃놀이 연출 시작
...
class GameOver(state.State):
    def on_enter(self):
        move_user(mapId=84000007, portalId=3, boxId=9002)
        ...
        play_system_sound_in_box(boxIds=[9010,9011,9012,9013], sound='DJDD_Ending_02') # action name="업적이벤트를발생시킨다" arg1="9001" arg2="trigger" arg3="ddstop_win"/
...
class GiveReward(state.State):
    def on_enter(self):
        end_mini_game(winnerBoxId=9001, gameName='WDdancedancestop') # action name="아이템을생성한다" arg1="7000,7001,7002,7003,7004,7005,7006,7007,7008,7009,7010,7011,7012,7013,7014,7015,7016,7017,7018,7019,7020,7021,7022,7023,7024,7025,7026,7027,7028,7029,7030,7031,7032,7033,7034,7035,7036,7037,7038,7039,7040,7041,7042,7043,7044,7045,7046,7047,7048,7049,7050,7051,7052,7053,7054,7055,7056,7057,7058,7059,7060,7061,7062,7063,7064,7065,7066,7067,7068,7069,7070,7071,7072,7073,7074,7075,7076,7077,7078,7079,7080,7081,7082,7083,7084"/
...
class FailAll(state.State):
    def on_enter(self):
        end_mini_game(winnerBoxId=9001, gameName='WDdancedancestop')
...
class LeaveAll(state.State):
    def on_enter(self):
        move_user(mapId=84000007, portalId=3, boxId=9001) # 종료 후 펀타임을 위해 전부 스테이지 밖으로 킥
        add_buff(boxIds=[9000], skillId=99940042, level=1, arg4=False, arg5=False) # 도입부부터 폭죽 터트리고 놀 수 있게 변경
        ...
        set_event_ui(type=1, arg2='$84000007_WD__01_MASSIVEMAIN__0$', arg3='10000') # Voice 02000970
        play_system_sound_in_box(boxIds=[9000], sound='DJDD_Ending_03')
        set_sound(triggerId=10000, arg2=False) # Intro
        set_timer(timerId='8888', seconds=150, clearAtZero=False, display=True) # 60초 동안 자유시간
        set_photo_studio(isEnable=True) # 신규개발기능: 사진촬영

    def on_tick(self) -> state.State:
        if time_expired(timerId='8888'):
            return Quit()
