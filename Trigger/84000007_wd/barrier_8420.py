...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10000951], state=2) # On
        set_interact_object(triggerIds=[10000967], state=2) # Off
...
class Sensor7422(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[8421,8422,8423,8424,8425,8426], visible=False, arg3=0, arg4=0, arg5=0)
        set_effect(triggerIds=[8420], visible=False)
        set_interact_object(triggerIds=[10000951], state=0) # On
        set_interact_object(triggerIds=[10000967], state=0) # Off
...
class Enable7422(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10000951], state=1) # On

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10000951], arg2=0):
            return Activate7422()
        if not count_users(boxId=9420, boxId=2, operator='Equal'):
            return Sensor7422()
        if user_value(key='Barrier42', value=10):
...
class Delay7422(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10000967], state=1) # Off
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='Barrier42', value=10):
            return Reset()
        if object_interacted(interactIds=[10000967], arg2=0):
