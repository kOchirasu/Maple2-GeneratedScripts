...
class 대기(state.State):
    def on_enter(self):
        ...
        set_actor(triggerId=12, visible=True, initialSequence='Closed')
...
class 몬스터와전투(state.State):
    def on_enter(self):
        set_actor(triggerId=12, visible=True, initialSequence='Opened')
...
class 몬스터전투(state.State):
    def on_tick(self) -> state.State:
        if monster_dead(boxIds=[103]):
            return 트리거초기화()
        if not monster_in_combat(boxIds=[103]):
            return 몬스터소멸()
...
class 몬스터소멸(state.State):
    ...
    def on_tick(self) -> state.State:
        if monster_in_combat(boxIds=[103]):
