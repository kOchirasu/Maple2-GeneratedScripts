...
class 낮시간으로변화대기단계(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1400):
            return 낮시간으로변화하기(self.ctx)


class 낮시간으로변화하기(common.Trigger):
    def on_enter(self):
...
class 낮시간으로변화하기_맵초기화(common.Trigger):
    def on_enter(self):
...
class 밤시간으로변화대기단계(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1400):
            return 밤시간으로변화하기(self.ctx)


class 밤시간으로변화하기(common.Trigger):
    def on_enter(self):
