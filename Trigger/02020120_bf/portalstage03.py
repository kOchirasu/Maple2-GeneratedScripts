...
class Ready(common.Trigger):
    def on_enter(self):
        ...
        self.set_user_value(key='Stage03', value=0) # 어느지점 포탈을 활성화 시킬지 결정하는데 사용하는 변수
...
class 스테이지3_시작(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='Stage03', value=11):
            return 스테이지3_왼쪽_왼쪽진행(self.ctx)
        if self.user_value(key='Stage03', value=12):
            return 스테이지3_왼쪽_가운데진행(self.ctx)
        if self.user_value(key='Stage03', value=21):
            return 스테이지3_가운데_왼쪽진행(self.ctx)
        if self.user_value(key='Stage03', value=22):
            return 스테이지3_가운데_가운데진행(self.ctx)
        if self.user_value(key='Stage03', value=23):
            return 스테이지3_가운데_오른쪽진행(self.ctx)
        if self.user_value(key='Stage03', value=31):
            return 스테이지3_오른쪽_가운데진행(self.ctx)
        if self.user_value(key='Stage03', value=32):
            return 스테이지3_오른쪽_오른쪽진행(self.ctx)


class 스테이지3_왼쪽_왼쪽진행(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=3101, visible=True, enable=True, minimapVisible=True) # 3스테이지로 가는 포탈 생성
        self.set_portal(portalId=3102, visible=True, enable=True, minimapVisible=True) # 3스테이지로 가는 포탈 생성
...
class 스테이지3_왼쪽_가운데진행(common.Trigger):
    def on_enter(self):
...
class 스테이지3_가운데_왼쪽진행(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=3201, visible=True, enable=True, minimapVisible=True) # 3스테이지로 가는 포탈 생성
...
class 스테이지3_가운데_가운데진행(common.Trigger):
    def on_enter(self):
...
class 스테이지3_가운데_오른쪽진행(common.Trigger):
    def on_enter(self):
...
class 스테이지3_오른쪽_가운데진행(common.Trigger):
    def on_enter(self):
...
class 스테이지3_오른쪽_오른쪽진행(common.Trigger):
    def on_enter(self):
