...
class 시작대기중(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[1920,1921,1922,1923,1924,1925,1926,1927,1928,1929,1930,1931,1932,1933,1934,1935,1936,1937], visible=False) # 몬스터는 밟을 수 있고 플레이어는 밟을 수 없는 투명벽 설정하기
...
class 보스등장(state.State):
    def on_enter(self):
        dungeon_reset_time(seconds=720) # 던전 시간 12분 설정 초기화 하기 , 이 던전의 시간 설정은 DoungeonRoom.xml 이 아닌 여기서 진행함
        ...
        set_portal(portalId=9901, visible=False, enabled=False, minimapVisible=False) # 버그 때문에 이거 사용 안함, 그렇다고 이거 지우면 대박 버그 생기니 조심 그냥 내버려 두자, 다시 처음으로 되돌리는 순간이동 포탈 Off 처리 하는거 여기서 하기, 스킬브레이크 막기 실패하여 던전 초기화 될 경우를 대비해 여기서 초기화 처리 하는 것이 좋음, 보스 메인 전투판 주변 범위의 포발
        set_portal(portalId=9902, visible=False, enabled=False, minimapVisible=False) # 버그 때문에 이거 사용 안함, 그렇다고 이거 지우면 대박 버그 생기니 조심 그냥 내버려 두자, 다시 처음으로 되돌리는 순간이동 포탈 Off 처리 하는거 여기서 하기, 스킬브레이크 막기 실패하여 던전 초기화 될 경우를 대비해 여기서 초기화 처리 하는 것이 좋음, 스타팅 포인트 근처 주변 범위의 포발
...
class 던전초기화진행(state.State):
    def on_enter(self):
        move_user(mapId=2020120, portalId=9903) # 맵 안에 있는 모든 플레이어를 스타팅 지점에 있는 포탈로 이동시킴, arg1 은 이동시킬 맵 코드, arg2 은 도착 장소 포탈ID 임
        set_sound(triggerId=19601, arg2=True) # 던전 초기화 되면 노말 BGM으로 교체함, 보스 BGM 교체 설정은 BgmChangeSkillBreakReset.xml 트리거에 설정 되어있음
        side_npc_talk(type='talk', npcId=23000113, illust='Ishura_Dark_smile', script='$02020120_BF__BOSSSPAWN__1$', duration=7000, voice='ko/Npc/00002193')
...
class 던전초기화시간등각종설정(state.State):
    def on_enter(self):
        dungeon_stop_timer()
        set_user_value(key='DungeonReset', value=0) # 스킬브레이크 실패하여 보스의 신호를 받아서 던전 리셋할때 사용하는 변수 0 초기화 하기
...
class 종료딜레이(state.State):
    ...
    def on_tick(self) -> state.State:
        if wait_tick(waitTick=8000):
            set_skill(triggerIds=[2222], isEnable=False)
            set_skill(triggerIds=[1212], isEnable=False)
            set_portal(portalId=2, visible=True, enabled=True, minimapVisible=True)
            dungeon_clear()
            dungeon_set_end_time()
            set_achievement(achieve='IshuraDungeonClear')
            return 종료()
...
class 종료(state.State):
    def on_enter(self):
        set_portal(portalId=2101, visible=True, enabled=True, minimapVisible=True)
        set_portal(portalId=2201, visible=True, enabled=True, minimapVisible=True)
        set_portal(portalId=2301, visible=True, enabled=True, minimapVisible=True)
        set_portal(portalId=3101, visible=True, enabled=True, minimapVisible=True)
        set_portal(portalId=3102, visible=True, enabled=True, minimapVisible=True)
        ...
        set_portal(portalId=3201, visible=True, enabled=True, minimapVisible=True)
        ...
        set_portal(portalId=4101, visible=True, enabled=True, minimapVisible=True)
        set_portal(portalId=4102, visible=True, enabled=True, minimapVisible=True)
        set_portal(portalId=4201, visible=True, enabled=True, minimapVisible=True)
        ...
        set_portal(portalId=5101, visible=True, enabled=True, minimapVisible=True)
        set_portal(portalId=5102, visible=True, enabled=True, minimapVisible=True)
        ...
        set_portal(portalId=6101, visible=True, enabled=True, minimapVisible=True)
