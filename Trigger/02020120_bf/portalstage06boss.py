...
class Ready(common.Trigger):
    def on_enter(self):
        ...
        self.set_user_value(key='Stage06', value=0) # 어느지점 포탈을 활성화 시킬지 결정하는데 사용하는 변수

    def on_tick(self) -> common.Trigger:
        if self.user_detected(boxIds=[199]):
            return 스테이지6_시작(self.ctx)


class 스테이지6_시작(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='Stage06', value=11):
            return 스테이지6_왼쪽_가운데진행(self.ctx)
        if self.user_value(key='Stage06', value=21):
            return 스테이지6_가운데_가운데진행(self.ctx)
        if self.user_value(key='Stage06', value=31):
            return 스테이지6_오른쪽_가운데진행(self.ctx)


class 스테이지6_왼쪽_가운데진행(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=6101, visible=True, enable=True, minimapVisible=True) # 6스테이지 보스로 가는 포탈 생성
...
class 스테이지6_가운데_가운데진행(common.Trigger):
    def on_enter(self):
...
class 스테이지6_오른쪽_가운데진행(common.Trigger):
    def on_enter(self):
