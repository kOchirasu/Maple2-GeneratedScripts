...
class 에르다사망대기(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[2002]):
            return 에르다사망딜레이(self.ctx)


class 에르다사망딜레이(common.Trigger):
    def on_enter(self):
...
class 에르다공중부양준비(common.Trigger):
    def on_enter(self):
...
class PC말풍선대사01(common.Trigger):
    def on_enter(self):
...
class 설눈이말풍선대사01(common.Trigger):
    def on_enter(self):
...
class PC말풍선대사02(common.Trigger):
    def on_enter(self):
...
class PC말풍선대사03(common.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[602], visible=False)
