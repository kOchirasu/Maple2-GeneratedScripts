...
class Setting(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002051], state=0) # Lever_BlockStart
        set_interact_object(triggerIds=[10002052], state=0) # Lever_BlockOff
...
class BlockEnable(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002051], state=1) # Lever_BlockStart

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10002051], arg2=0):
            return BlockStart()
...
class BlockDisable(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002052], state=1) # Lever_BlockOff

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10002052], arg2=0):
