...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10002046], state=0) # Chair
...
class GuideFindPortal(state.State):
    def on_enter(self):
        ...
        show_guide_summary(entityId=20039706, textId=20039706) # 가이드 : 다른 방으로 이동할 단서를 찾으세요
...
class MobTrapOnEnd(state.State):
    def on_tick(self) -> state.State:
...
class SwichOn(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002046], state=1) # LeverForLadder01

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10002046], arg2=0):
            return RockMove01()
