...
class 대기(state.State):
    def on_enter(self):
        set_timer(timerId='59', seconds=180, clearAtZero=True, display=True, arg5=-90)
...
class 어나운스0(state.State):
    def on_enter(self):
        ...
        set_event_ui(type=1, arg2='$61000004_ME__TRIGGER_01__0$', arg3='4000', arg4='0')
...
class 어나운스1(state.State):
    ...
    def on_tick(self) -> state.State:
        if wait_tick(waitTick=5500):
            ...
            start_mini_game(isShowResultUI=False, boxId=105, round=1, gameName='UserMassive_Escape')
...
class 경기종료(state.State):
    def on_enter(self):
        ...
        set_event_ui(type=4, arg2='$61000004_ME__TRIGGER_01__3$', arg3='5000', arg4='!102')
        add_buff(boxIds=[102], skillId=70000132, level=1)
        add_buff(boxIds=[102], skillId=70000019, level=1) # 에레브의 축복 버프 걸어줌

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=6000):
            end_mini_game_round(winnerBoxId=102, expRate=0.13333334, isGainLoserBonus=True, gameName='UserMassive_Escape')
            end_mini_game(winnerBoxId=102, gameName='UserMassive_Escape')
            return 강제이동()
