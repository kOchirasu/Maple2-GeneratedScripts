...
class GameGuide01(state.State):
    def on_enter(self):
        ...
        set_achievement(triggerId=9001, type='trigger', achieve='ddstop_start') # 길드 경험치 지급 / boxID="타겟박스id", 0이면 맵전체, type="GuildGainExp의 id" 2가 매시브이벤트
        give_guild_exp(boxId=0, type=2)
        set_mini_game_area_for_hack(boxId=9001) # 해킹 보안용 시작 box 설정
        start_mini_game(boxId=9001, round=5, gameName='dancedancestop')
...
class R05End(state.State):
    def on_enter(self):
        write_log(logName='dancedancestop', event='9001', arg3='round_clear', arg4='5')
        set_cinematic_ui(type=0)
        set_cinematic_ui(type=2)
        select_camera(triggerId=901, enable=False)
...
class GameOver(state.State):
    def on_enter(self):
        move_user(mapId=61000008, portalId=3, boxId=9002)
...
class GiveReward(state.State):
    def on_enter(self):
        ...
        end_mini_game(winnerBoxId=9001, gameName='dancedancestop') # action name="아이템을생성한다" arg1="7000,7001,7002,7003,7004,7005,7006,7007,7008,7009,7010,7011,7012,7013,7014,7015,7016,7017,7018,7019,7020,7021,7022,7023,7024,7025,7026,7027,7028,7029,7030,7031,7032,7033,7034,7035,7036,7037,7038,7039,7040,7041,7042,7043,7044,7045,7046,7047,7048,7049,7050,7051,7052,7053,7054,7055,7056,7057,7058,7059,7060,7061,7062,7063,7064,7065,7066,7067,7068,7069,7070,7071,7072,7073,7074,7075,7076,7077,7078,7079,7080,7081,7082,7083,7084"/
...
class FailAll(state.State):
    def on_enter(self):
        end_mini_game(winnerBoxId=9001, gameName='dancedancestop')
