...
class Camera_Move_01(common.Trigger):
    def on_enter(self):
...
class NPC_Change_1(common.Trigger):
    def on_enter(self):
...
class NPC_Attack_Move(common.Trigger):
    def on_enter(self):
...
class teleport02000487(common.Trigger):
    def on_enter(self):
        self.set_skip()
        self.destroy_monster(spawnIds=[-1])
        self.visible_my_pc(isVisible=True)
        self.move_user(mapId=2000487, portalId=3) # 프론티어 재단 저택 맵 3번 회의실 앞 포탈
