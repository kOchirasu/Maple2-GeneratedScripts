...
# 		7. x분 후 자동으로 결혼식장 복귀
# 유저 입장
class Reception(common.Trigger):
    def on_enter(self):
...
# 가이드5: 콘대르가 미션 제공2
class GameGuide05(common.Trigger):
    def on_enter(self):
...
class GameGuide06(common.Trigger):
    def on_enter(self):
...
# 전투 페이즈1: 훔쳐먹기 60초
class Pinata_Ready(common.Trigger):
    def on_enter(self):
...
class Pinata_Fight(common.Trigger):
    def on_enter(self):
...
# 전투 페이즈2: 악령잡기
class Pinata_Fight2(common.Trigger):
    def on_enter(self):
...
# 전투종료 : 악령퇴치 성공
class Pinata_Kill(common.Trigger):
    def on_enter(self):
...
# 전투종료 : 악령퇴치 실패
class Pinata_noKill(common.Trigger):
    def on_enter(self):
        self.destroy_monster(spawnIds=[201]) # 악령 피냐타 사라짐
...
class Pinata_Revive(common.Trigger):
    def on_enter(self):
...
class Pinata_Revive2(common.Trigger):
    def on_enter(self):
        self.set_ambient_light(primary=[255,255,255]) # 조명 원상 복구
        self.set_user_value(triggerId=1004, key='Interaction', value=2) # UV발사: IntObj OFF
...
# 불꽃놀이 타임
class Pinata_Fireworks(common.Trigger):
    def on_enter(self):
