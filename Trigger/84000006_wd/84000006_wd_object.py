...
class UserCount(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.count_users(box_id=9001, min_users='50', operator='GreaterEqual'):
            # 50인이상
            return Over50(self.ctx)
        if self.count_users(box_id=9001, min_users='30', operator='GreaterEqual'):
            # 30인이상
            return Over30(self.ctx)
        if self.count_users(box_id=9001, min_users='30', operator='Less'):
            # 30인미만
...
class Over50(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interact_ids=[10001442], ar2='2') and self.object_interacted(interact_ids=[10001443], ar2='2') and self.object_interacted(interact_ids=[10001444], ar2='2') and self.object_interacted(interact_ids=[10001445], ar2='2'):
            # 종료조건: 시간경과 or 전부훔쳐먹음
            self.add_buff(box_ids=[9002], skill_id=99940046, level=1, is_player=False, is_skill_set=True) # 달팽이 스킬셋 50인용 제공
...
class Over30(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        # all_of: 난이도 조절로 꺼둠
        # all_of: 테스트용 1개
        if self.object_interacted(interact_ids=[10001442], ar2='2') and self.object_interacted(interact_ids=[10001443], ar2='2') and self.object_interacted(interact_ids=[10001445], ar2='2'):
...
class Under30(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        # all_of: 난이도 조절로 꺼둠
        # all_of: 테스트용 꼬다리 1개
        if self.object_interacted(interact_ids=[10001442], ar2='2') and self.object_interacted(interact_ids=[10001445], ar2='2'):
            self.add_buff(box_ids=[9002], skill_id=99940043, level=1, is_player=False, is_skill_set=True) # 달팽이 스킬셋 30Under 제공
            self.set_user_value(trigger_id=1001, key='Steal', value=1) # 전투 페이즈2 이동 UV발사
            return Standby(self.ctx)
        if self.time_expired(timer_id='2'):
            self.add_buff(box_ids=[9002], skill_id=99940043, level=1, is_player=False, is_skill_set=True) # 달팽이 스킬셋 제공
            self.set_user_value(trigger_id=1001, key='Steal', value=1) # 전투 페이즈2 이동 UV발사
            return Standby(self.ctx)
