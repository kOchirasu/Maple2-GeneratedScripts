...
class 입장(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # <일반 5분 타이머>
        self.set_user_value(triggerId=99990006, key='SpecialTimer', value=0)
        # <3분내 클리어 특별 타이머>
...
class 준비(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.npc_detected(boxId=902, spawnIds=[201]) and self.wait_tick(waitTick=2000):
...
class 전투_진행(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # self.set_user_value(triggerId=99990006, key='SpecialTimer', value=1)
