...
class 라운드조건체크(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.dungeon_round() == 12:
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_normal', script='$83000003_COLOSSEUM__ROUND12__0$', duration=5000)
            # self.set_event_ui(type=1, arg2='전투에서 승리하셨습니다. 이제 부터는 벅찬 상대가 나올 수 있습니다. 마음 단단히 먹으십시오.', arg3='3000')
            return 라운드대기(self.ctx)
        self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000003_COLOSSEUM__ROUND12__1$', duration=3000)
...
class 몬스터스폰(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_buff(box_ids=[112], skill_id=69000501, level=1, is_player=True)
...
class 스폰대사(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(spawn_ids=[112]):
            ...
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[112])
        ...
        if self.time_expired(timer_id='LimitTimer'):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000003_COLOSSEUM__ROUND12__5$', duration=3000)
        ...
        if self.user_detected(box_ids=[902]):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000003_COLOSSEUM__ROUND12__6$', duration=3000)
            self.destroy_monster(spawn_ids=[112])
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[112])
            self.destroy_monster(spawn_ids=[10000])
            self.destroy_monster(spawn_ids=[10001])
        ...
        if not self.user_detected(box_ids=[904]):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000003_COLOSSEUM__ROUND12__7$', duration=3000)
            self.destroy_monster(spawn_ids=[112])
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[112])
            self.destroy_monster(spawn_ids=[10000])
            self.destroy_monster(spawn_ids=[10001])
...
class ClearRound(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3000):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_normal', script='$83000003_COLOSSEUM__ROUND12__10$', duration=3000)
