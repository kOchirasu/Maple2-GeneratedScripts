...
class ContinuePlay(state.State):
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='ClearRound', value=10):
            set_user_value(triggerId=910011, key='StartRound11', value=1)
            return ResultRound11()
        if user_value(key='ClearRound', value=11):
            set_user_value(triggerId=910012, key='StartRound12', value=1)
            return ResultRound12()
        if user_value(key='ClearRound', value=12):
            set_user_value(triggerId=910013, key='StartRound13', value=1)
            return ResultRound13()
        if user_value(key='ClearRound', value=13):
            return WaitRound1()
...
class ResultRound11(state.State):
    def on_tick(self) -> state.State:
        if user_value(key='StartRound11', value=2):
            dungeon_clear_round(round=11)
            set_user_value(triggerId=910011, key='StartRound11', value=0)
            set_user_value(triggerId=900001, key='StartRound11', value=0)
            return WaitRound12()
        if user_value(key='StartRound11', value=3):
            set_user_value(triggerId=910011, key='StartRound11', value=0)
            set_user_value(triggerId=900001, key='StartRound11', value=0)
            return RoundClear10()


class WaitRound12(state.State):
    def on_tick(self) -> state.State:
        if wait_tick(waitTick=3000):
            set_user_value(triggerId=910012, key='StartRound12', value=1)
            return ResultRound12()
...
class ResultRound12(state.State):
    def on_tick(self) -> state.State:
        if user_value(key='StartRound12', value=2):
            dungeon_clear_round(round=12)
            set_user_value(triggerId=910012, key='StartRound12', value=0)
            set_user_value(triggerId=900001, key='StartRound12', value=0)
            return WaitRound13()
        if user_value(key='StartRound12', value=3):
            set_user_value(triggerId=910012, key='StartRound12', value=0)
            set_user_value(triggerId=900001, key='StartRound12', value=0)
            return RoundClear11()


class WaitRound13(state.State):
    def on_tick(self) -> state.State:
        if wait_tick(waitTick=3000):
            set_user_value(triggerId=910013, key='StartRound13', value=1)
            return ResultRound13()
...
class ResultRound13(state.State):
    def on_tick(self) -> state.State:
        if user_value(key='StartRound13', value=2):
            dungeon_clear_round(round=13)
            set_user_value(triggerId=910013, key='StartRound13', value=0)
            set_user_value(triggerId=900001, key='StartRound13', value=0)
            return AllRoundClear()
        if user_value(key='StartRound13', value=3):
            set_user_value(triggerId=910013, key='StartRound13', value=0)
            set_user_value(triggerId=900001, key='StartRound13', value=0)
            return RoundClear12()
...
class RoundClear10(state.State):
    def on_enter(self):
        dungeon_clear(uiType='None')
...
class RoundClear11(state.State):
    def on_enter(self):
        dungeon_clear(uiType='None')
...
class RoundClear12(state.State):
    def on_enter(self):
        dungeon_clear(uiType='None')
