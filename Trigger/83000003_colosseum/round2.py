...
class 라운드조건체크(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.dungeon_round() == 2:
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND2__0$', duration=5000)
            # self.set_event_ui(type=1, arg2='전투에서 승리하셨습니다. 잠시만 기다려주세요 다음전투가 기다리고 있습니다!!', arg3='3000')
            return 라운드대기(self.ctx)
        self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND2__1$', duration=3000)
...
class 몬스터스폰(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_buff(box_ids=[102], skill_id=69000501, level=1, is_player=True)
...
class 스폰대사(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(spawn_ids=[102]):
            ...
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[102])
        ...
        if self.time_expired(timer_id='LimitTimer'):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND2__5$', duration=3000)
        ...
        if self.user_detected(box_ids=[902]):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND2__6$', duration=3000)
            self.destroy_monster(spawn_ids=[102])
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[102])
            return FailRoundDelay(self.ctx)
        if not self.user_detected(box_ids=[904]):
            # self.set_event_ui(type=1, arg2='패배했습니다. 전투가 종료됩니다.', arg3='3000')
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND2__7$', duration=3000)
            self.destroy_monster(spawn_ids=[102])
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[102])
            return FailRoundDelay(self.ctx)
...
class ClearRound(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3000):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND2__10$', duration=3000)
