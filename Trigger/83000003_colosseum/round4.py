...
class 대기(common.Trigger):
    ...
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='StartRound4', value=1):
...
class 라운드조건체크(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.dungeon_round_require(round=4):
            self.side_npc_talk(type='talk', npcId=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND4__0$', duration=5000)
            return 라운드대기(self.ctx)
        if self.true():
            self.side_npc_talk(type='talk', npcId=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND4__1$', duration=3000)
...
class 라운드대기(common.Trigger):
    ...
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=5000):
            self.side_npc_talk(type='cutin', illust='Alon_normal', duration=3000)
...
class 몬스터스폰(common.Trigger):
    def on_enter(self):
        ...
        self.add_buff(boxIds=[104], skillId=69000501, level=1, isPlayer=True)
...
class 카운트(common.Trigger):
    def on_enter(self):
        self.show_count_ui(text='$83000002_COLOSSEUM__ROUND4__2$', count=3, soundType=2)
...
class 스폰대사(common.Trigger):
    def on_enter(self):
        self.add_balloon_talk(spawnId=104, msg='$83000002_COLOSSEUM__ROUND4__3$', duration=3000)
        self.set_timer(timerId='LimitTimer', seconds=120, startDelay=1)
        self.set_npc_duel_hp_bar(isOpen=True, spawnId=[104], durationTick=120000, npcHpStep=10)

    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[104]):
            self.add_balloon_talk(spawnId=104, msg='$83000002_COLOSSEUM__ROUND4__4$', duration=3000)
            self.set_npc_duel_hp_bar(isOpen=False, spawnId=[104])
            return ClearRoundDelay(self.ctx)
        if self.time_expired(timerId='LimitTimer'):
            self.side_npc_talk(type='talk', npcId=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND4__5$', duration=3000)
            self.destroy_monster(spawnIds=[104])
            self.set_npc_duel_hp_bar(isOpen=False, spawnId=[104])
            return FailRoundDelay(self.ctx)
        if self.user_detected(boxIds=[902]):
            self.side_npc_talk(type='talk', npcId=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND4__6$', duration=3000)
            self.destroy_monster(spawnIds=[104])
            self.set_npc_duel_hp_bar(isOpen=False, spawnId=[104])
            return FailRoundDelay(self.ctx)
        if not self.user_detected(boxIds=[904]):
            self.side_npc_talk(type='talk', npcId=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND4__7$', duration=3000)
            self.destroy_monster(spawnIds=[104])
            self.set_npc_duel_hp_bar(isOpen=False, spawnId=[104])
            return FailRoundDelay(self.ctx)
...
class ClearRound(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=3000):
            ...
            self.side_npc_talk(type='talk', npcId=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND4__10$', duration=3000)
            self.set_user_value(triggerId=900001, key='StartRound4', value=2)
...
class FailRound(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=3000):
            self.set_user_value(triggerId=900001, key='StartRound4', value=3)
            return 대기(self.ctx)
