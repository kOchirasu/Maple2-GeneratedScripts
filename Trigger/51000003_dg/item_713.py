...
class Spawn_check(state.State):
    def on_enter(self):
        set_effect(triggerIds=[7713], visible=False)

    def on_tick(self) -> state.State:
        if user_value(key='item_713_spawn', value=1):
            return SpawnItem()


class SpawnItem(state.State):
    def on_enter(self):
        set_effect(triggerIds=[7713], visible=True)
        set_effect(triggerIds=[7723], visible=False)

    def on_tick(self) -> state.State:
        if user_detected(boxIds=[723]):
            return GetItem_Random()
        if user_value(key='item_713_spawn', value=0):
            return Spawn_check()
...
class GetItem_Random(state.State):
    def on_enter(self):
        ...
        set_effect(triggerIds=[7713], visible=False)
        set_effect(triggerIds=[7723], visible=True)
