...
class Spawn_check(state.State):
    def on_enter(self):
        set_effect(triggerIds=[7714], visible=False)
        set_effect(triggerIds=[7724], visible=False)

    def on_tick(self) -> state.State:
        if user_value(key='item_714_spawn', value=1):
            return SpawnItem()


class SpawnItem(state.State):
    def on_enter(self):
        set_effect(triggerIds=[7714], visible=True)

    def on_tick(self) -> state.State:
        if user_detected(boxIds=[724]):
            return GetItem_Random()
        if user_value(key='item_714_spawn', value=0):
            return Spawn_check()
...
class GetItem_Random(state.State):
    def on_enter(self):
        ...
        set_effect(triggerIds=[7714], visible=False)
        set_effect(triggerIds=[7724], visible=True)
