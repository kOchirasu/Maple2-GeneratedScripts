...
class Spawn_check(trigger_api.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[7717], visible=False)
        self.set_effect(triggerIds=[7727], visible=False)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='item_717_spawn', value=1):
            return SpawnItem(self.ctx)


class SpawnItem(trigger_api.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[7717], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(boxIds=[727]):
            return GetItem_Random(self.ctx)
        if self.user_value(key='item_717_spawn', value=0):
...
class GetItem_Random(trigger_api.Trigger):
    def on_enter(self):
        ...
        self.set_effect(triggerIds=[7717], visible=False)
        self.set_effect(triggerIds=[7727], visible=True)
