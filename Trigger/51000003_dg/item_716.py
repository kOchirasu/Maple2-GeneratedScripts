...
class Spawn_check(state.State):
    def on_enter(self):
        set_effect(triggerIds=[7716], visible=False)
        set_effect(triggerIds=[7726], visible=False)

    def on_tick(self) -> state.State:
        if user_value(key='item_716_spawn', value=1):
            return SpawnItem()


class SpawnItem(state.State):
    def on_enter(self):
        set_effect(triggerIds=[7716], visible=True)

    def on_tick(self) -> state.State:
        if user_detected(boxIds=[726]):
            return GetItem_Random()
        if user_value(key='item_716_spawn', value=0):
            return Spawn_check()
...
class GetItem_Random(state.State):
    def on_enter(self):
        ...
        set_effect(triggerIds=[7716], visible=False)
        set_effect(triggerIds=[7726], visible=True)
