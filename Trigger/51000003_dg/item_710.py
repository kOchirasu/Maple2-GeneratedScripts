...
class Spawn_check(trigger_api.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[7710], visible=False)
        self.set_effect(triggerIds=[7720], visible=False)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='item_710_spawn', value=1):
            return SpawnItem(self.ctx)


class SpawnItem(trigger_api.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[7710], visible=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(boxIds=[720]):
            return GetItem_Random(self.ctx)
        if self.user_value(key='item_710_spawn', value=0):
...
class GetItem_Random(trigger_api.Trigger):
    def on_enter(self):
        ...
        self.set_effect(triggerIds=[7710], visible=False)
        self.set_effect(triggerIds=[7720], visible=True)
...
class Buff_01(trigger_api.Trigger):
    ...
    def on_exit(self):
        self.set_user_value(triggerId=991119, key='item_710_spawn', value=0)
