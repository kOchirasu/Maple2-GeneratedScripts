...
class Spawn_check(state.State):
    def on_enter(self):
        set_effect(triggerIds=[7710], visible=False)
        set_effect(triggerIds=[7720], visible=False)

    def on_tick(self) -> state.State:
        if user_value(key='item_710_spawn', value=1):
            return SpawnItem()


class SpawnItem(state.State):
    def on_enter(self):
        set_effect(triggerIds=[7710], visible=True)

    def on_tick(self) -> state.State:
        if user_detected(boxIds=[720]):
            return GetItem_Random()
        if user_value(key='item_710_spawn', value=0):
            return Spawn_check()
...
class GetItem_Random(state.State):
    def on_enter(self):
        ...
        set_effect(triggerIds=[7710], visible=False)
        set_effect(triggerIds=[7720], visible=True)
...
class Buff_01(state.State):
    ...
    def on_exit(self):
        set_user_value(triggerId=991119, key='item_710_spawn', value=0)
