...
class main(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.count_users(boxId=90001, minUsers='1'):
...
class 단계_시작4(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.create_monster(spawnIds=[150,151], animationEffect=False)
        # 1,2차 웨이브 몬스터 작동 장치
...
class 클리어_보상_02(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_timer(timerId='3', seconds=3)
        # self.set_interact_object(triggerIds=[13000012], state=1)
        # 보상 상태 개방
