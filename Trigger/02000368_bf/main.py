...
class 전투감지(state.State):
    def on_tick(self) -> state.State:
        if monster_dead(boxIds=[101,111,201,211,301,311,401,411,501,511,601,611]):
            return 전투감지2()


class 전투감지2(state.State):
    def on_tick(self) -> state.State:
        if monster_dead(boxIds=[102,202,302,402,502,602]):
            return 포털개방()
...
class 포털개방(state.State):
    def on_enter(self):
        ...
        set_portal(portalId=40, visible=True, enabled=True, minimapVisible=False)
        set_portal(portalId=50, visible=True, enabled=True, minimapVisible=False)
        set_portal(portalId=60, visible=True, enabled=True, minimapVisible=False)
