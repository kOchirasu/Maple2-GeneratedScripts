...
class Wait(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[2], jobCode=50):
            ...
            return Quit(self.ctx)
        if self.quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[1], jobCode=50):
            # 초보자 넬프의 죽음 퀘스트 수락
            return LoadingDelay02(self.ctx)
        if self.quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[3], jobCode=50):
            # 초보자 랄프의 정보 퀘스트 완료
            return DefaultSetting02(self.ctx)
        if self.quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[2], jobCode=50):
