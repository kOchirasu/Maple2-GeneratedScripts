...
class Wait(state.State):
    ...
    def on_tick(self) -> state.State:
        if quest_user_detected(boxIds=[9000], questIds=[60100070], questStates=[3], jobCode=20):
            return Quit()
        if quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[2], jobCode=20):
            return Quit()
        if quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[1], jobCode=20):
            return LoadingDelay02()
        if quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[3], jobCode=20):
            return DefaultSetting02()
        if quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[2], jobCode=20):
...
class Quit02(state.State):
    def on_enter(self):
        ...
        set_event_ui(type=1, arg2='$52000037_QD__LOOKINTO_BERSERKER_04__0$', arg3='3000', arg4='0')
