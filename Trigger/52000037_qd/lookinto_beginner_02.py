...
class Wait(common.Trigger):
    ...
    def on_tick(self) -> common.Trigger:
        if self.quest_user_detected(boxIds=[9000], questIds=[60100070], questStates=[3], jobCode=1):
            return Quit(self.ctx)
        if self.quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[2], jobCode=1):
            return Quit(self.ctx)
        if self.quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[1], jobCode=1):
            return LoadingDelay02(self.ctx)
        if self.quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[3], jobCode=1):
            return DefaultSetting02(self.ctx)
        if self.quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[2], jobCode=1):
