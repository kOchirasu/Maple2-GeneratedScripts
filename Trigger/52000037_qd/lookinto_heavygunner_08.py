...
class Wait(state.State):
    ...
    def on_tick(self) -> state.State:
        if quest_user_detected(boxIds=[9000], questIds=[60100070], questStates=[3], jobCode=60):
            return Quit()
        if quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[2], jobCode=60):
            return Quit()
        if quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[1], jobCode=60):
            return LoadingDelay02()
        if quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[3], jobCode=60):
            return DefaultSetting02()
        if quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[2], jobCode=60):
...
class Quit02(state.State):
    def on_enter(self):
        ...
        set_event_ui(type=1, arg2='$52000037_QD__LOOKINTO_HEAVYGUNNER_08__0$', arg3='3000', arg4='0')
