...
class Wait(common.Trigger):
    ...
    def on_tick(self) -> common.Trigger:
        if self.quest_user_detected(boxIds=[9000], questIds=[60100070], questStates=[3], jobCode=60):
            return Quit(self.ctx)
        if self.quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[2], jobCode=60):
            return Quit(self.ctx)
        if self.quest_user_detected(boxIds=[9900], questIds=[60100070], questStates=[1], jobCode=60):
            return LoadingDelay02(self.ctx)
        if self.quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[3], jobCode=60):
            return DefaultSetting02(self.ctx)
        if self.quest_user_detected(boxIds=[9000], questIds=[60100065], questStates=[2], jobCode=60):
...
class CameraFadeIn01(common.Trigger):
    def on_enter(self):
        self.set_onetime_effect(id=1, enable=True, path='BG/Common/ScreenMask/Eff_fadein_1sec.xml')
...
class CameraFadeOut01(common.Trigger):
    def on_enter(self):
        self.set_onetime_effect(id=1, enable=False, path='BG/Common/ScreenMask/Eff_fadein_1sec.xml')
