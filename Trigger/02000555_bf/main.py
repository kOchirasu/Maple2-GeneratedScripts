...
class 스테이지1_추가등장대기01(common.Trigger):
    def on_enter(self):
...
class 스테이지1_추가등장01(common.Trigger):
    def on_enter(self):
...
class 스테이지1_대기01(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='MonsterMany', value=6, operator='LessEqual'):
            return 스테이지1_추가등장02(self.ctx)


class 스테이지1_추가등장02(common.Trigger):
    def on_enter(self):
...
class 스테이지1_대기02(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='MonsterMany', value=6, operator='LessEqual'):
            return 스테이지1_추가등장03(self.ctx)


class 스테이지1_추가등장03(common.Trigger):
    def on_enter(self):
...
class 스테이지2_대기02(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='MonsterMany', value=0):
            return 스테이지2문파괴대기_스테이지3몬스터등장(self.ctx)


class 스테이지2문파괴대기_스테이지3몬스터등장(common.Trigger):
    def on_enter(self):
        self.set_user_value(key='MonsterMany', value=0) # MonsterMany변수 0으로 초기 셋팅, 소환몹이 한마리 나올때마다 1 더하고 한마리씩 죽으몬 1 빼기 함, 몬스터 등장 이전에 이 변수 0 초기화 하기
...
class 스테이지3_진행중(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='MonsterMany', value=0):
            return 스테이지3문파괴대기_스테이지4몬스터등장(self.ctx)


class 스테이지3문파괴대기_스테이지4몬스터등장(common.Trigger):
    def on_enter(self):
        self.set_user_value(key='MonsterMany', value=0) # MonsterMany변수 0으로 초기 셋팅, 소환몹이 한마리 나올때마다 1 더하고 한마리씩 죽으몬 1 빼기 함, 몬스터 등장 이전에 이 변수 0 초기화 하기
...
class 스테이지4_진행중(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='MonsterMany', value=0):
            return 스테이지4문파괴대기_스테이지5몬스터등장(self.ctx)


class 스테이지4문파괴대기_스테이지5몬스터등장(common.Trigger):
    def on_enter(self):
        self.set_user_value(key='MonsterMany', value=0) # MonsterMany변수 0으로 초기 셋팅, 소환몹이 한마리 나올때마다 1 더하고 한마리씩 죽으몬 1 빼기 함, 몬스터 등장 이전에 이 변수 0 초기화 하기
...
class 스테이지5_완료대기(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='MonsterMany', value=0):
            return 스테이지5_완료(self.ctx)


class 스테이지5_완료(common.Trigger):
    def on_enter(self):
