...
class Wait(trigger_api.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000203], state=2) # RareBox 맵 별로 유니크하도록 변경해야 하는 값
...
class EndMelodyQuiz_Success(trigger_api.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000203], state=1) # RareBox 맵 별로 유니크하도록 변경해야 하는 값
        self.set_timer(timerId='2', seconds=60, startDelay=1, interval=0, vOffset=0)

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interactIds=[12000203], stateValue=0):
