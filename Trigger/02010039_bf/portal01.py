...
class 대기(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10000886], state=1)

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10000886], arg2=0):
            return 생성()


class 생성(state.State):
    def on_enter(self):
        set_portal(portalId=60, visible=False, enabled=True, minimapVisible=False)
