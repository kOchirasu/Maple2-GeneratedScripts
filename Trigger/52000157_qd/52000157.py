...
class 떠나기전준비(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.quest_user_detected(boxIds=[2002], questIds=[40002731], questStates=[2]):
            return 이별준비(self.ctx)


class 이별준비(common.Trigger):
    def on_enter(self):
