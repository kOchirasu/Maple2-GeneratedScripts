...
class 대기(common.Trigger):
    def on_enter(self):
        ...
        self.set_portal(portalId=7, visible=False, enable=False, minimapVisible=False) # 2페이지에 있는 나가기 포탈, 최초에는 감추기
        self.set_portal(portalId=2, visible=False, enable=False, minimapVisible=False) # 3페이지에 있는 나가기 포탈, 최초에는 감추기
...
class 클리어처리01(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=6000):
            return 클리어처리02(self.ctx)


class 클리어처리02(common.Trigger):
    def on_enter(self):
        self.destroy_monster(spawnIds=[-1])
        self.dungeon_clear() # DungeonRoom.xlsx  의  clearType  trigger 로 설정한 경우 맵 트리거를 통해서 클리어 UI 띄우는 처리함
...
class 나가기포털생성(common.Trigger):
    def on_enter(self):
        self.set_user_value(triggerId=99999002, key='BattleEnd', value=1) # move.xml  트리거에 BattleEnd = 1 신호 보내기
        ...
        self.set_portal(portalId=6, visible=True, enable=True, minimapVisible=True) # 1페이지 왼쪽 전투판에 등장하는 나가기 포탈 생성
        self.set_portal(portalId=7, visible=True, enable=True, minimapVisible=True) # 1페이지 오른쪽 전투판에 등장하는 나가기 포탈 생성
        self.set_portal(portalId=2, visible=True, enable=True, minimapVisible=True) # 2페이지 전투판에 등장하는 나가기 포탈 생성
        self.set_portal(portalId=3, visible=True, enable=True, minimapVisible=True) # 3페이지 전투판에 등장하는 나가기 포탈 생성
