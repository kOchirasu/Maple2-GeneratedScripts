...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[34011], visible=True, start_delay=0, interval=0, fade=0)
...
class 발판(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=200):
            self.set_mesh(trigger_ids=[34011], visible=False, start_delay=0, interval=0, fade=0)
            return 종료(self.ctx)
