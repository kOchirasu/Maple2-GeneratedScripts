...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[12000223], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001121 걸어서 71001021 제거
...
class TracingFootHold_Success(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[12000223], state=1) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001121 걸어서 71001021 제거
        set_timer(timerId='2', seconds=60, clearAtZero=True, display=False, arg5=0)

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[12000223], arg2=0):
            return TracingFootHold_SuccessDelay()
        if time_expired(timerId='2'):
            set_interact_object(triggerIds=[12000223], state=2)
            return ResetTimer()
