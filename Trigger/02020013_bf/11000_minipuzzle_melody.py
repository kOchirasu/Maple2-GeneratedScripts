...
class Wait(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000201], state=2) # RareBox 맵 별로 유니크하도록 변경해야 하는 값
...
class EndMelodyQuiz_Success(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000201], state=1) # RareBox 맵 별로 유니크하도록 변경해야 하는 값
        self.set_timer(timerId='2', seconds=60, startDelay=1, interval=0, vOffset=0)

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[12000201], stateValue=0):
