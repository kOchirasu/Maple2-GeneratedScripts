...
class 아이들과만남_연출대기(common.Trigger):
    def on_enter(self):
...
class 아이들과만남_연출시작(common.Trigger):
    def on_enter(self):
...
class 유저맵이동_연출종료(common.Trigger):
    def on_enter(self):
        self.reset_camera(interpolationTime=3)
        self.destroy_monster(spawnIds=[101,102])
...
class 아이들과인사_스킵완료(common.Trigger):
    def on_enter(self):
        self.set_onetime_effect(id=1, enable=True, path='BG/Common/ScreenMask/Eff_fadein_1sec.xml')
        self.reset_camera(interpolationTime=3)
        self.destroy_monster(spawnIds=[101,102])
