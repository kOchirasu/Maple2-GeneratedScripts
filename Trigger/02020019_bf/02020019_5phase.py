...
class 크림슨발록스폰체크(common.Trigger):
    def on_enter(self):
        self.add_buff(feature='FameChallengeBuff_01', boxIds=[241], skillId=49218001, level=1, isPlayer=True) # <한국용 공격력 1.2배 강화 버프>
        ...
        self.add_buff(feature='FameChallengeBuff_02', boxIds=[241], skillId=49218002, level=1, isPlayer=True) # <중국용 공격력 2배 강화 버프>
...
class 크림슨스피어죽음(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[241]):
            return 발록에게신호쏴주기(self.ctx)


class 발록에게신호쏴주기(common.Trigger):
    def on_enter(self):
