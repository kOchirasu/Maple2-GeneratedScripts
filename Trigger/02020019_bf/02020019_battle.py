...
class 전투_5라운드버프(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.dungeon_check_play_time(playSeconds=180, operator='LessEqual') and self.monster_dead(boxIds=[241,242,243,244,245,246,247,248]):
            # <한국용 컨디션체크>
            self.dungeon_mission_complete(missionId=24093004)
            self.dungeon_mission_complete(missionId=24093005)
            return 전투_종료(self.ctx)
        if self.dungeon_check_play_time(playSeconds=70, operator='LessEqual') and self.monster_dead(boxIds=[241,242,243,244,245,246,247,248]):
            # <중국용 컨디션체크>
            self.dungeon_mission_complete(missionId=24093004)
            self.dungeon_mission_complete(missionId=24093006)
            return 전투_종료(self.ctx)
        if self.dungeon_check_play_time(playSeconds=270, operator='LessEqual') and self.monster_dead(boxIds=[241,242,243,244,245,246,247,248]):
            # <NA용 컨디션체크>
            self.dungeon_mission_complete(missionId=24093004)
...
class 전투_종료(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.destroy_monster(spawnIds=[221,222,223,224,225,226,227])
        self.destroy_monster(spawnIds=[231,232,233,234,235,236,237])
        self.destroy_monster(spawnIds=[241,242,243,244,245,246,247,248])
