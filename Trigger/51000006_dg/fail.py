...
class Fail(trigger_api.Trigger):
    def on_enter(self):
        ...
        self.write_log(logName='ThreeTwoOne_log', triggerId=9001, event='char_event', subEvent='BlackbeanThreeTwoOnegameover') # 로그를 남기기 위한 행 : arg5가 트리거 전체에서 유니크한 값이 들어가야 하며, arg1은 코드에 남고 있지 않음(서바이벌일 때만 서바이벌 로그 불러옴)
