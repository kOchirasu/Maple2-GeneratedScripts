...
class 게임시작_대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.write_log(logName='ThreeTwoOne_log', triggerId=9000, event='char_event', subEvent='BlackbeanThreeTwoOnegamestart')
        # lifeCount : 최대 사망 횟수
...
class 라운드시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.arcade_three_two_one3(type='StartRound', uiDuration=4, round=1)
...
class 결과연출(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='ThreeTwoOneResult', value=1):
            ...
            self.set_npc_emotion_sequence(spawnId=101, sequenceName='Dance_C_Ride', durationTick=3300) # 블랙빈 돌아서서 고개숙이고 씩씩 3300
            self.set_pc_emotion_loop(sequenceName='Emotion_Dance_E', duration=3300) # PC 신나서 댄스
            self.set_effect(triggerIds=[602], visible=True) # PC 머리 위 스포트라이트 이펙트
            # 이펙트 604: PC 머리 위 불꽃놀이 이펙트
            self.set_effect(triggerIds=[604], visible=True)
            self.add_balloon_talk(spawnId=101, msg='$51000006_DG__51000006_MAIN__6$', duration=3300) # 블랙빈 : 내가 졌잖아?!
        ...
        if self.user_value(key='ThreeTwoOneResult', value=0):
            ...
            self.set_effect(triggerIds=[601], visible=True) # 블랙빈 머리 위 스포트라이트 이펙트
            # 이펙트 603: 게임상대 - 11004718 블랙빈 머리 위 불꽃놀이 이펙트
            self.set_effect(triggerIds=[603], visible=True)
            self.set_effect(triggerIds=[610], visible=True) # 이펙트 610: PC 머리 위에 비
            self.set_actor(triggerId=611, visible=True, initialSequence='Run_A') # 약올리는미니빈01 ON
            self.set_actor(triggerId=612, visible=True, initialSequence='Run_A') # 약올리는미니빈02 ON
            self.set_actor(triggerId=613, visible=True, initialSequence='Run_A') # 약올리는미니빈03 ON
            self.set_actor(triggerId=614, visible=True, initialSequence='Run_A') # 약올리는미니빈04 ON
            self.set_pc_emotion_sequence(sequenceNames=['Emotion_Fuss_A']) # PC 패배
            self.add_balloon_talk(spawnId=101, msg='$51000006_DG__51000006_MAIN__8$', duration=3300) # …블랙빈 : 얼레리꼴레리~ 졌대요~ 메롱!
