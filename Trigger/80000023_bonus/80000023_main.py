...
class 입장(common.Trigger):
    def on_enter(self):
        ...
        self.create_monster(spawnIds=[101,102,103,104,105], animationEffect=False) # 101-4: 용병 우르자들 / 105:연출용 블랙빈
...
class 안내(common.Trigger):
    def on_enter(self):
        self.show_guide_summary(entityId=1, textId=26300739, duration=10)
...
class 몬스터체크(common.Trigger):
    def on_enter(self):
        self.show_guide_summary(entityId=1, textId=26300739, duration=10000) # 가이드 텍스트 ON : 시간여행자 블랙빈의 용병, 험상궂은 우르자들을 처치하세요!
...
class 길을열어라(common.Trigger):
    def on_enter(self):
        self.set_npc_emotion_sequence(spawnId=105, sequenceName='Dead_A') # 블랙빈 빠잉
...
class 문열기00(common.Trigger):
    def on_enter(self):
        self.show_guide_summary(entityId=2, textId=26300740, duration=10000) # 가이드 텍스트 ON : 문이 열렸어요. 출구에 있는 빈빈 코인을 주운 후 이동하세요.
...
class 문열기06(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[193,195,197], visible=False) # 뒷문 / 열기
        self.destroy_monster(spawnIds=[105]) # 블랙빈 소멸
