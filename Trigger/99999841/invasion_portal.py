...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(trigger_ids=[10002184], state=2, arg3=False)


class 포탈열림(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_timer(timer_id='1', seconds=30, start_delay=1)
        self.set_interact_object(trigger_ids=[10002184], state=1, arg3=False)
...
class 유저이동(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.dungeon_variable(var_id=2, value=1):
            return 종료(self.ctx)
        if self.dungeon_variable(var_id=1000, value=1):
