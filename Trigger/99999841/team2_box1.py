...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.dungeon_variable(var_id=911, value=0)
        self.set_interact_object(trigger_ids=[10002179], state=0, arg3=False)

    def on_tick(self) -> trigger_api.Trigger:
        if self.dungeon_variable(var_id=901, value=1):
            return 종료(self.ctx)
        if self.dungeon_variable(var_id=902, value=1):
            return 종료(self.ctx)
        if self.dungeon_variable(var_id=903, value=1):
            return 종료(self.ctx)
...
class 애디셔널_중첩1(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.dungeon_variable(var_id=911, value=1)
