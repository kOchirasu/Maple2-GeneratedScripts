...
class 레버4_몬스터등장(common.Trigger):
    def on_enter(self):
...
class 레버4_재활성_대기(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='ObjectStart', value=0):
            return 대기(self.ctx)
        if self.monster_dead(boxIds=[701]):
