...
class 보스소환1(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.user_value(key='ObjectStart', value=2) and self.npc_detected(boxId=9099, spawnIds=[921]):
...
class 보스소환2(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.monster_dead(boxIds=[922]):
            return 보스군단_클리어(self.ctx)
        if self.user_value(key='ObjectStart', value=2) and self.npc_detected(boxId=9099, spawnIds=[922]):
            return 보스_무적페이즈(self.ctx)
...
class 보스소환3(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.monster_dead(boxIds=[923]):
            return 보스군단_클리어(self.ctx)
        if self.user_value(key='ObjectStart', value=2) and self.npc_detected(boxId=9099, spawnIds=[923]):
            return 보스_무적페이즈(self.ctx)
...
class 보스소환4(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.monster_dead(boxIds=[924]):
            return 보스군단_클리어(self.ctx)
        if self.user_value(key='ObjectStart', value=2) and self.npc_detected(boxId=9099, spawnIds=[924]):
            return 보스_무적페이즈(self.ctx)
...
class 보스소환5(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.monster_dead(boxIds=[925]):
            return 보스군단_클리어(self.ctx)
        if self.user_value(key='ObjectStart', value=2) and self.npc_detected(boxId=9099, spawnIds=[925]):
            return 보스_무적페이즈(self.ctx)
...
class 보스소환6(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.monster_dead(boxIds=[926]):
            return 보스군단_클리어(self.ctx)
        if self.user_value(key='ObjectStart', value=2) and self.npc_detected(boxId=9099, spawnIds=[926]):
            return 보스_무적페이즈(self.ctx)
...
class 보스_무적페이즈(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='BossPhase', value=2):
            return 대기(self.ctx)
        if self.monster_dead(boxIds=[921]) and self.monster_dead(boxIds=[922]) and self.monster_dead(boxIds=[923]) and self.monster_dead(boxIds=[924]) and self.monster_dead(boxIds=[925]) and self.monster_dead(boxIds=[926]):
            return 보스군단_클리어(self.ctx)
        if self.wait_tick(waitTick=5000):
