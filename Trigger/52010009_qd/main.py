...
class Event_01(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        ...
        self.set_interact_object(triggerIds=[10000866], state=1)
        # 컨테이너를 반응 가능한 상태로 변경
...
class Event_03(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.hide_guide_summary(entityId=110)
        self.set_interact_object(triggerIds=[10000880], state=1)
        # 컨테이너를 반응 가능한 상태로 변경
...
class Event_05(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.hide_guide_summary(entityId=110)
        self.set_interact_object(triggerIds=[10000915], state=1)
        # 컨테이너를 반응 가능한 상태로 변경
