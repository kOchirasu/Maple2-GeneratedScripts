...
class 대기(state.State):
    def on_enter(self):
        set_actor(triggerId=1007, visible=False, initialSequence='Closed')
...
class 준비(state.State):
    ...
    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10000529,10000520], arg2=0):
            return 트리거02시작()


class 트리거02시작(state.State):
    def on_enter(self):
        set_actor(triggerId=1007, visible=True, initialSequence='Opened')
        create_monster(spawnIds=[2034], arg2=True)
