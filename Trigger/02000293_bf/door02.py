...
class 대기(state.State):
    def on_enter(self):
        set_actor(triggerId=1002, visible=False, initialSequence='Closed')
...
class 준비(state.State):
    ...
    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10000524,10000505], arg2=0):
            return 트리거02시작()


class 트리거02시작(state.State):
    def on_enter(self):
        set_actor(triggerId=1002, visible=True, initialSequence='Opened')
        create_monster(spawnIds=[2029], arg2=True)
