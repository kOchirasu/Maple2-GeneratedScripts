...
class 대기(trigger_api.Trigger):
    def on_enter(self):
        self.set_actor(triggerId=1002, visible=False, initialSequence='Closed')
...
class 준비(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interactIds=[10000524,10000505], stateValue=0):
            return 트리거02시작(self.ctx)


class 트리거02시작(trigger_api.Trigger):
    def on_enter(self):
        self.set_actor(triggerId=1002, visible=True, initialSequence='Opened')
        self.create_monster(spawnIds=[2029], animationEffect=True)
