...
class 스폰_2(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.time_expired(timer_id='2') or self.score_board_compare(operator='GreaterEqual', score=250):
            return 스폰_3_SE(self.ctx)
        if self.user_value(key='Battle_3_SpawnStart', value=0):
            return 대기(self.ctx)
...
class 스폰_3(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.time_expired(timer_id='3') or self.score_board_compare(operator='GreaterEqual', score=500):
...
class 스폰_4(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.time_expired(timer_id='4') or self.score_board_compare(operator='GreaterEqual', score=750):
