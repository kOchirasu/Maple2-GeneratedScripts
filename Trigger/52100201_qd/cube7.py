...
class 큐브7(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4010], visible=True, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[5010], visible=True, arg3=0, delay=0, scale=0)

    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=5000):
            self.set_user_value(triggerId=920007, key='CubeOff', value=1)
...
class 큐브제거(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4010], visible=False, arg3=0, delay=0, scale=0)
    ...
    def on_tick(self) -> common.Trigger:
        ...
        if self.user_detected(boxIds=[9004]):
            self.set_user_value(triggerId=910007, key='Cube', value=0)
            return 종료(self.ctx)
