...
class 큐브17(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4029,4030,4031,4032], visible=True, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[5029,5030,5031,5032], visible=True, arg3=0, delay=0, scale=0)

    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=5000):
            self.set_user_value(triggerId=920017, key='CubeOff', value=1)
...
class 큐브제거(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4029,4030,4031,4032], visible=False, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[5029,5030,5031,5032], visible=False, arg3=0, delay=0, scale=0)

    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=5000):
            return 큐브17(self.ctx)
        if self.user_detected(boxIds=[9004]):
            self.set_user_value(triggerId=910017, key='Cube', value=0)
            return 종료(self.ctx)
