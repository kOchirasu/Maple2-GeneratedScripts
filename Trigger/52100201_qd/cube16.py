...
class 큐브16(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[4028], visible=True, arg3=0, arg4=0, arg5=0)
        set_mesh(triggerIds=[5028], visible=True, arg3=0, arg4=0, arg5=0)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=5000):
            set_user_value(triggerId=920016, key='CubeOff', value=1)
            return 메쉬제거()
...
class 큐브제거(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[4028], visible=False, arg3=0, arg4=0, arg5=0)
    ...
    def on_tick(self) -> state.State:
        ...
        if user_detected(boxIds=[9004]):
            set_user_value(triggerId=910016, key='Cube', value=0)
            return 종료()
