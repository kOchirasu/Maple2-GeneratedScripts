...
class 큐브제거(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4018], visible=False, arg3=0, delay=0, scale=0)

    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=2800):
            return 큐브10(self.ctx)
        if self.user_detected(boxIds=[9004]):
            self.set_user_value(triggerId=910010, key='Cube', value=0)
            return 종료(self.ctx)
