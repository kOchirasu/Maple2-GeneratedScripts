...
class 큐브8(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[4011,4012,4013], visible=True, arg3=0, arg4=0, arg5=0)
        set_mesh(triggerIds=[5011,5012,5013], visible=True, arg3=0, arg4=0, arg5=0)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=5000):
            set_user_value(triggerId=920008, key='CubeOff', value=1)
            return 메쉬제거()
...
class 큐브제거(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[4011,4012,4013], visible=False, arg3=0, arg4=0, arg5=0)
        set_mesh(triggerIds=[5011,5012,5013], visible=False, arg3=0, arg4=0, arg5=0)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=3800):
            return 큐브8()
        if user_detected(boxIds=[9004]):
            set_user_value(triggerId=910008, key='Cube', value=0)
            return 종료()
