...
class 큐브11(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[4019], visible=True, arg3=0, arg4=0, arg5=0)
        set_mesh(triggerIds=[5019], visible=True, arg3=0, arg4=0, arg5=0)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=5000):
            set_user_value(triggerId=920011, key='CubeOff', value=1)
            return 메쉬제거()
...
class 큐브제거(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[4019], visible=False, arg3=0, arg4=0, arg5=0)
        set_mesh(triggerIds=[5019], visible=False, arg3=0, arg4=0, arg5=0)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=1900):
            return 큐브11()
        if user_detected(boxIds=[9004]):
            set_user_value(triggerId=910011, key='Cube', value=0)
            return 종료()
