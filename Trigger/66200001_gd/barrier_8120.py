...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001186], state=2) # On
        set_interact_object(triggerIds=[10001202], state=2) # Off
...
class Sensor7122(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[8121,8122,8123,8124,8125,8126], visible=False, arg3=0, arg4=0, arg5=0)
        set_effect(triggerIds=[8120], visible=False)
        set_interact_object(triggerIds=[10001186], state=0) # On
        set_interact_object(triggerIds=[10001202], state=0) # Off
...
class CheckSameUserTag7122(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7122()
        if not count_users(boxId=9120, boxId=2, operator='Equal'):
            return Sensor7122()
        if not check_same_user_tag(boxId=9120):
            return SafeGreen7122()
        if user_value(key='Barrier12', value=10):
            return Reset()
...
class Enable7122(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001186], state=1) # On

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001186], arg2=0):
            return Activate7122()
        if not count_users(boxId=9120, boxId=2, operator='Equal'):
            return Sensor7122()
        if user_value(key='Barrier12', value=10):
...
class Delay7122(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001202], state=1) # Off
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='Barrier12', value=10):
            return Reset()
        if object_interacted(interactIds=[10001202], arg2=0):
...
class CheckSameUserTag7123(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7123()
        if not count_users(boxId=9120, boxId=3, operator='Equal'):
...
class CheckSameUserTag7124(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7124()
        if not count_users(boxId=9120, boxId=4, operator='Equal'):
...
class CheckSameUserTag7125(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7125()
        if not count_users(boxId=9120, boxId=5, operator='Equal'):
