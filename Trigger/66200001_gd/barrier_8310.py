...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001193], state=2) # On
        set_interact_object(triggerIds=[10001209], state=2) # Off
...
class Sensor7312(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[8311,8312,8313,8314,8315,8316], visible=False, arg3=0, arg4=0, arg5=0)
        set_effect(triggerIds=[8310], visible=False)
        set_interact_object(triggerIds=[10001193], state=0) # On
        set_interact_object(triggerIds=[10001209], state=0) # Off
...
class CheckSameUserTag7312(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7312()
        if not count_users(boxId=9310, boxId=2, operator='Equal'):
            return Sensor7312()
        if not check_same_user_tag(boxId=9310):
            return SafeGreen7312()
        if user_value(key='Barrier31', value=10):
            return Reset()
...
class Enable7312(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001193], state=1) # On

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001193], arg2=0):
            return Activate7312()
        if not count_users(boxId=9310, boxId=2, operator='Equal'):
            return Sensor7312()
        if user_value(key='Barrier31', value=10):
...
class Delay7312(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001209], state=1) # Off
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='Barrier31', value=10):
            return Reset()
        if object_interacted(interactIds=[10001209], arg2=0):
...
class CheckSameUserTag7313(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7313()
        if not count_users(boxId=9310, boxId=3, operator='Equal'):
...
class CheckSameUserTag7314(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7314()
        if not count_users(boxId=9310, boxId=4, operator='Equal'):
...
class CheckSameUserTag7315(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7315()
        if not count_users(boxId=9310, boxId=5, operator='Equal'):
