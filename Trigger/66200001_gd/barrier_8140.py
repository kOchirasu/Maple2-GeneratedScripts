...
class Wait(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(triggerIds=[10001188], state=2) # On
        self.set_interact_object(triggerIds=[10001204], state=2) # Off
...
class Sensor7142(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(triggerIds=[8141,8142,8143,8144], visible=False, arg3=0, delay=0, scale=0)
        self.set_effect(triggerIds=[8140], visible=False)
        self.set_interact_object(triggerIds=[10001188], state=0) # On
        self.set_interact_object(triggerIds=[10001204], state=0) # Off
...
class CheckSameUserTag7142(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_same_user_tag(boxId=9140) and self.count_users(boxId=9140, minUsers='2', operator='Equal'):
            return Enable7142(self.ctx)
        if not self.count_users(boxId=9140, minUsers='2', operator='Equal'):
            return Sensor7142(self.ctx)
        if not self.check_same_user_tag(boxId=9140):
            return SafeGreen7142(self.ctx)
        if self.user_value(key='Barrier14', value=10):
            return Reset(self.ctx)
...
class Enable7142(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(triggerIds=[10001188], state=1) # On

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interactIds=[10001188], stateValue=0):
            # On
...
class Delay7142(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_interact_object(triggerIds=[10001204], state=1) # Off
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.user_value(key='Barrier14', value=10):
            return Reset(self.ctx)
        if self.object_interacted(interactIds=[10001204], stateValue=0):
            # Off
...
class CheckSameUserTag7143(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_same_user_tag(boxId=9140) and self.count_users(boxId=9140, minUsers='3', operator='Equal'):
...
class CheckSameUserTag7144(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_same_user_tag(boxId=9140) and self.count_users(boxId=9140, minUsers='4', operator='Equal'):
...
class CheckSameUserTag7145(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_same_user_tag(boxId=9140) and self.count_users(boxId=9140, minUsers='5', operator='Equal'):
