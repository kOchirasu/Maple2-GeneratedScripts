...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001188], state=2) # On
        set_interact_object(triggerIds=[10001204], state=2) # Off
...
class Sensor7142(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[8141,8142,8143,8144], visible=False, arg3=0, arg4=0, arg5=0)
        set_effect(triggerIds=[8140], visible=False)
        set_interact_object(triggerIds=[10001188], state=0) # On
        set_interact_object(triggerIds=[10001204], state=0) # Off
...
class CheckSameUserTag7142(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7142()
        if not count_users(boxId=9140, boxId=2, operator='Equal'):
            return Sensor7142()
        if not check_same_user_tag(boxId=9140):
            return SafeGreen7142()
        if user_value(key='Barrier14', value=10):
            return Reset()
...
class Enable7142(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001188], state=1) # On

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001188], arg2=0):
            return Activate7142()
        if not count_users(boxId=9140, boxId=2, operator='Equal'):
            return Sensor7142()
        if user_value(key='Barrier14', value=10):
...
class Delay7142(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001204], state=1) # Off
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='Barrier14', value=10):
            return Reset()
        if object_interacted(interactIds=[10001204], arg2=0):
...
class CheckSameUserTag7143(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7143()
        if not check_same_user_tag(boxId=9140):
...
class CheckSameUserTag7144(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7144()
        if not count_users(boxId=9140, boxId=4, operator='Equal'):
...
class CheckSameUserTag7145(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7145()
        if not count_users(boxId=9140, boxId=5, operator='Equal'):
