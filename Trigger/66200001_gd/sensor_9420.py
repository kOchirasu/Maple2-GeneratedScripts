...
class Wait(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(triggerIds=[542], visible=True, arg3=0, delay=0, scale=0) # 42 / Ground outter
        self.set_mesh(triggerIds=[5420], visible=True, arg3=0, delay=0, scale=0) # 42 / Ground inner

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='Box42Check', value=0):
            return Sensor0(self.ctx)
        if self.user_value(key='Box42Check', value=1):
            return Sensor1(self.ctx)
        if self.user_value(key='Box42Check', value=2):
            return Sensor2(self.ctx)
        if self.user_value(key='Box42Check', value=3):
            return Sensor3(self.ctx)
        if self.user_value(key='Box42Check', value=4):
            return Sensor4(self.ctx)
        if self.user_value(key='Box42Check', value=5):
...
class Pass(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.play_system_sound_in_box(boxIds=[9420], sound='DDStop_Stage_Pass_01')
        self.set_mesh(triggerIds=[542], visible=False, arg3=0, delay=0, scale=2) # 42 / Ground outter
        self.set_user_value(triggerId=7420, key='ColorReset', value=1) # color reset
...
class Fail(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.play_system_sound_in_box(boxIds=[9420], sound='DDStop_Stage_Fail_01')
        self.set_mesh(triggerIds=[542], visible=False, arg3=0, delay=0, scale=2) # 42 / Ground outter
        self.set_mesh(triggerIds=[5420], visible=False, arg3=0, delay=0, scale=0) # 42 / Ground inner
        self.set_user_value(triggerId=7420, key='ColorClear', value=1) # color clear
