...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001192], state=2) # On
        set_interact_object(triggerIds=[10001208], state=2) # Off
...
class Sensor7242(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[8241,8242,8243,8244,8245,8246], visible=False, arg3=0, arg4=0, arg5=0)
        set_effect(triggerIds=[8240], visible=False)
        set_interact_object(triggerIds=[10001192], state=0) # On
        set_interact_object(triggerIds=[10001208], state=0) # Off
...
class CheckSameUserTag7242(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7242()
        if not count_users(boxId=9240, boxId=2, operator='Equal'):
            return Sensor7242()
        if not check_same_user_tag(boxId=9240):
            return SafeGreen7242()
        if user_value(key='Barrier24', value=10):
            return Reset()
...
class Enable7242(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001192], state=1) # On

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001192], arg2=0):
            return Activate7242()
        if not count_users(boxId=9240, boxId=2, operator='Equal'):
            return Sensor7242()
        if user_value(key='Barrier24', value=10):
...
class Delay7242(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001208], state=1) # Off
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='Barrier24', value=10):
            return Reset()
        if object_interacted(interactIds=[10001208], arg2=0):
...
class CheckSameUserTag7243(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7243()
        if not count_users(boxId=9240, boxId=3, operator='Equal'):
...
class CheckSameUserTag7244(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7244()
        if not count_users(boxId=9240, boxId=4, operator='Equal'):
...
class CheckSameUserTag7245(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7245()
        if not count_users(boxId=9240, boxId=5, operator='Equal'):
