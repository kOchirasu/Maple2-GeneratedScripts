...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001187], state=2) # On
        set_interact_object(triggerIds=[10001203], state=2) # Off
...
class Sensor7132(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[8131,8132,8133,8134,8135,8136], visible=False, arg3=0, arg4=0, arg5=0)
        set_effect(triggerIds=[8130], visible=False)
        set_interact_object(triggerIds=[10001187], state=0) # On
        set_interact_object(triggerIds=[10001203], state=0) # Off
...
class CheckSameUserTag7132(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7132()
        if not count_users(boxId=9130, boxId=2, operator='Equal'):
            return Sensor7132()
        if not check_same_user_tag(boxId=9130):
            return SafeGreen7132()
        if user_value(key='Barrier13', value=10):
            return Reset()
...
class Enable7132(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001187], state=1) # On

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001187], arg2=0):
            return Activate7132()
        if not count_users(boxId=9130, boxId=2, operator='Equal'):
            return Sensor7132()
        if user_value(key='Barrier13', value=10):
...
class Delay7132(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001203], state=1) # Off
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='Barrier13', value=10):
            return Reset()
        if object_interacted(interactIds=[10001203], arg2=0):
...
class CheckSameUserTag7133(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7133()
        if not count_users(boxId=9130, boxId=3, operator='Equal'):
...
class CheckSameUserTag7134(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7134()
        if not count_users(boxId=9130, boxId=4, operator='Equal'):
...
class CheckSameUserTag7135(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7135()
        if not count_users(boxId=9130, boxId=5, operator='Equal'):
