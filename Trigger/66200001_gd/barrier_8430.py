...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001199], state=2) # On
        set_interact_object(triggerIds=[10001215], state=2) # Off
...
class Sensor7432(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[8431,8432,8433,8434,8435,8436], visible=False, arg3=0, arg4=0, arg5=0)
        set_effect(triggerIds=[8430], visible=False)
        set_interact_object(triggerIds=[10001199], state=0) # On
        set_interact_object(triggerIds=[10001215], state=0) # Off
...
class CheckSameUserTag7432(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7432()
        if not count_users(boxId=9430, boxId=2, operator='Equal'):
            return Sensor7432()
        if not check_same_user_tag(boxId=9430):
            return SafeGreen7432()
        if user_value(key='Barrier43', value=10):
            return Reset()
...
class Enable7432(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001199], state=1) # On

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001199], arg2=0):
            return Activate7432()
        if not count_users(boxId=9430, boxId=2, operator='Equal'):
            return Sensor7432()
        if user_value(key='Barrier43', value=10):
...
class Delay7432(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001215], state=1) # Off
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='Barrier43', value=10):
            return Reset()
        if object_interacted(interactIds=[10001215], arg2=0):
...
class CheckSameUserTag7433(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7433()
        if not count_users(boxId=9430, boxId=3, operator='Equal'):
...
class CheckSameUserTag7434(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7434()
        if not count_users(boxId=9430, boxId=4, operator='Equal'):
...
class CheckSameUserTag7435(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7435()
        if not count_users(boxId=9430, boxId=5, operator='Equal'):
