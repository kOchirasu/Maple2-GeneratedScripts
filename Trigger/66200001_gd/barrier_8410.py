...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001197], state=2) # On
        set_interact_object(triggerIds=[10001213], state=2) # Off
...
class Sensor7412(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[8411,8412,8413,8414], visible=False, arg3=0, arg4=0, arg5=0)
        set_effect(triggerIds=[8410], visible=False)
        set_interact_object(triggerIds=[10001197], state=0) # On
        set_interact_object(triggerIds=[10001213], state=0) # Off
...
class CheckSameUserTag7412(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7412()
        if not count_users(boxId=9410, boxId=2, operator='Equal'):
            return Sensor7412()
        if not check_same_user_tag(boxId=9410):
            return SafeGreen7412()
        if user_value(key='Barrier41', value=10):
            return Reset()
...
class Enable7412(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001197], state=1) # On

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001197], arg2=0):
            return Activate7412()
        if not count_users(boxId=9410, boxId=2, operator='Equal'):
            return Sensor7412()
        if user_value(key='Barrier41', value=10):
...
class Delay7412(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001213], state=1) # Off
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='Barrier41', value=10):
            return Reset()
        if object_interacted(interactIds=[10001213], arg2=0):
...
class CheckSameUserTag7413(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7413()
        if not count_users(boxId=9410, boxId=3, operator='Equal'):
...
class CheckSameUserTag7414(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7414()
        if not count_users(boxId=9410, boxId=4, operator='Equal'):
...
class CheckSameUserTag7415(state.State):
    def on_tick(self) -> state.State:
        if all_of():
            return Enable7415()
        if not count_users(boxId=9410, boxId=5, operator='Equal'):
