...
class IsGameStartOrNot(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.count_users(boxId=9000, minUsers='10', operator='GreaterEqual', userTagId=1):
            # 1팀 10명 이상인지
            return IsGameStart_True01(self.ctx)
        if self.count_users(boxId=9000, minUsers='10', operator='GreaterEqual', userTagId=2):
            # 2팀 10명 이상인지
            return IsGameStart_Ture02(self.ctx)
        if self.count_users(boxId=9000, minUsers='10', operator='Less', userTagId=1):
            # 1팀 10명 미만이면
...
class IsGameStart_True01(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.count_users(boxId=9000, minUsers='10', operator='Less', userTagId=2):
            # 2팀 10명 미만이면, 1팀 부전승
...
class R01DanceTime(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.set_interact_object(triggerIds=[10001180], state=2)
        # 7000ms
...
class R01G00Check(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.count_users(boxId=9001, minUsers='50', operator='Greater'):
