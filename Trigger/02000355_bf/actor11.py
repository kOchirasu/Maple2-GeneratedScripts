...
class 대기(state.State):
    def on_enter(self):
        ...
        set_actor(triggerId=211, visible=True, initialSequence='Damg_B')
...
class 더미해제(state.State):
    def on_enter(self):
        set_actor(triggerId=211, visible=False, initialSequence='Damg_B')

    def on_tick(self) -> state.State:
        if monster_dead(boxIds=[2011]):
            return 소멸()
        if monster_dead(boxIds=[2099]):
            return 소멸()
        if npc_detected(boxId=105, spawnIds=[2011]):
...
class 리젠준비(state.State):
    def on_enter(self):
        set_actor(triggerId=211, visible=True, initialSequence='Regen_A')
