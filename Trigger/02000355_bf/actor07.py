...
class 대기(state.State):
    def on_enter(self):
        ...
        set_actor(triggerId=207, visible=True, initialSequence='Damg_B')

    def on_tick(self) -> state.State:
        if user_detected(boxIds=[1701]):
            return 몬스터소환대기()
...
class 더미해제(state.State):
    def on_enter(self):
        set_actor(triggerId=207, visible=False, initialSequence='Damg_B')
...
class 리젠준비(state.State):
    def on_enter(self):
        set_actor(triggerId=207, visible=True, initialSequence='Regen_A')
