...
class Wait(trigger_api.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000252], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001151 걸어서 71001051 제거
...
class InteractWithNpc_Success(trigger_api.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000252], state=1) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001151 걸어서 71001051 제거

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interactIds=[12000252], stateValue=0):
...
class InteractWithNpc_Fail(trigger_api.Trigger):
    def on_enter(self):
        self.set_interact_object(triggerIds=[12000251], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001151 걸어서 71001051 제거
        self.set_interact_object(triggerIds=[12000078], state=2) # NPC_TurnedAround / 기믹 시작 오브젝트 / TypeA / Additional Effect 71001051 71001231 부여
        self.set_interact_object(triggerIds=[12000093], state=2) # NPC_TurnedAround / 기믹 시작 오브젝트 / TypeB / Additional Effect 71001051 71001241 부여
