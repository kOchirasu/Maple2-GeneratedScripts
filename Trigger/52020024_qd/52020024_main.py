...
class 차전투감지1(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_detected(boxIds=[902]):
            return 차전투1(self.ctx)


class 차전투1(common.Trigger):
    def on_enter(self):
...
class 카메라_미카엘등장(common.Trigger):
    def on_enter(self):
...
class 카메라_미카엘대사1(common.Trigger):
    def on_enter(self):
...
class 카메라_미카엘대사2(common.Trigger):
    def on_enter(self):
...
class 카메라_바닥부서짐(common.Trigger):
    def on_enter(self):
