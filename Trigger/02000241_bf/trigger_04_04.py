...
class 대기(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[307], visible=True, arg3=0, arg4=0, arg5=0)
    ...
    def on_tick(self) -> state.State:
        if user_detected(boxIds=[407]):
            return 버튼눌림()


class 버튼눌림(state.State):
    def on_enter(self):
        set_mesh(triggerIds=[307], visible=False, arg3=0, arg4=0, arg5=0)
        set_actor(triggerId=511, visible=True, initialSequence='Opened')
        set_actor(triggerId=512, visible=True, initialSequence='Opened')
        create_monster(spawnIds=[611,612], arg2=False)
