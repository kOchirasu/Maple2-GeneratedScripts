...
class 작아짐제거(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_buff(boxIds=[702], skillId=50001556, level=1, isPlayer=False, isSkillSet=False)
        # 플레이어가 진공청소기 공격 받아서 작아졌는데, 흡수 안되는 이상한 버그가 있어서 만약을 위한 장치로 작아짐 걸렸으면, 여기서 풀어주도록 함
        self.add_buff(boxIds=[701], skillId=50001556, level=1, isPlayer=False, isSkillSet=False)
...
class 게임오버(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_buff(boxIds=[701], skillId=50000266, level=1, isPlayer=False, isSkillSet=False)
        # 플레이어에게 무적 버프 & 진공청소기 흡수에 의한 소인화 디버프 제거, 혹시 바닥 RYB 대미지에맞아서 클리어 했는데 죽는 경우를 막기 위해
        self.add_buff(boxIds=[702], skillId=50000266, level=1, isPlayer=False, isSkillSet=False)
