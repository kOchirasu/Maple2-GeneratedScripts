...
class 포탈체크시작(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1000):
            return 포탈체크대기중(self.ctx)


class 포탈체크대기중(common.Trigger):
    def on_tick(self) -> common.Trigger:
...
class 순간이동포탈OFF(common.Trigger):
    def on_enter(self):
...
class 나가기포탈생성잠시대기(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=9500):
            return 나가기포탈생성(self.ctx)


class 나가기포탈생성(common.Trigger):
    def on_tick(self) -> common.Trigger:
...
class 페이지전투판포탈생성1(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=2, visible=True, enable=True, minimapVisible=True) # 보스 죽이면 나가기 포탈 생성하기, 졸구간 전투판에서 나가기 포탈
...
class 페이지전투판포탈생성2(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=2, visible=True, enable=True, minimapVisible=True) # 보스 죽이면 나가기 포탈 생성하기, 졸구간 전투판에서 나가기 포탈
...
class 페이지전투판포탈생성3(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=2, visible=True, enable=True, minimapVisible=True) # 보스 죽이면 나가기 포탈 생성하기, 졸구간 전투판에서 나가기 포탈
