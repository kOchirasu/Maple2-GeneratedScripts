...
class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(trigger_ids=[201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,220,221,222,223], visible=False, start_delay=0, interval=0, fade=0)
        # 보스 전투판으로 진입하는 계단 트리거메쉬 처음에는 감추기
        self.set_mesh(trigger_ids=[101,102,103,104,105,106,107,108,109,110,111,112,113,114,115,116,117,118,119,120,121,122,123,124,125,126,127,128,129,130,131,132,133,134,135,136,137,138,139,140,141,142,143,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173,174,175,176,177,178], visible=False, start_delay=0, interval=0, fade=0)
        ...
        self.set_mesh(trigger_ids=[6701,6702,6703,6704,6705,6706,6707,6708,6709,6710,6711,6712,6713,6714,6715,6716,6717,6718,6719,6720,6721,6722,6723,6724,6725,6726,6727,6728,6729], visible=False, start_delay=0, interval=0, fade=0)
...
class 대기상태(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.user_value(key='2PhaseStart', value=1):
...
class 계단생성(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(trigger_ids=[198,199], visible=False, start_delay=0, interval=0, fade=0)
...
class 페이지_바로가기포탈생성2(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(trigger_ids=[125,116,168,126], visible=False, start_delay=0, interval=50, fade=0.5)
        # 무한 루프 빠지는 것을 막기 위해 이 변수 0 초기화 하기, 이후 이 변수 다시 사용할 일 없음
        ...
        self.set_mesh(trigger_ids=[198,199], visible=True, start_delay=1, interval=1, fade=1)
        # 2페이지 시작할 때 1페이지 전투판 파괴하기 때문에 1페이지 진입하는 계단 제거하기
        self.set_mesh(trigger_ids=[101,102,103,104,105,106,107,108,109,110,111,112,113,114,115,116,117,118,119,120,121,122,123,124,125,126,127,128,129,130,131,132,133,134,135,136,137,138,139,140,141,142,143,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173,174,175,176,177,178], visible=False, start_delay=0, interval=50, fade=0.5)
        """
...
class 페이지_2PhaseWall_생성2(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(trigger_ids=[6001,6002,6003,6004,6005,6006,6007,6008,6009,6010,6011,6012,6013,6014,6015,6016,6017,6018,6019,6020,6021,6022,6023,6024,6025,6026,6027,6028,6029,6030,6031,6032,6033,6034,6035,6036,6037,6038,6039,6040,6041,6042,6043,6044,6045,6046,6047,6048,6049,6050,6051,6052,6053,6054,6055,6056,6057,6058,6059,6060,6061,6062,6063,6064,6065,6066,6067,6068,6069,6070,6071,6072], visible=False, start_delay=0, interval=0, fade=0)
...
class 페이지_바로가기포탈생성3(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # 2페이지 드랍때 전투판 밖으로 떨어지는 플레이어 체크하는 포탈 3페이지 시작할때 꼭 제거하기
        self.set_portal(portal_id=61, visible=False, enable=False, minimap_visible=False)
        # 2페이지 드랍때 전투판 밖으로 떨어지는 플레이어 체크하는 포탈 3페이지 시작할때 꼭 제거하기
        self.set_portal(portal_id=62, visible=False, enable=False, minimap_visible=False)
        # 2페이지 드랍때 전투판 밖으로 떨어지는 플레이어 체크하는 포탈 3페이지 시작할때 꼭 제거하기
        self.set_portal(portal_id=63, visible=False, enable=False, minimap_visible=False)
        # 2페이지 드랍때 전투판 밖으로 떨어지는 플레이어 체크하는 포탈 3페이지 시작할때 꼭 제거하기
        self.set_portal(portal_id=64, visible=False, enable=False, minimap_visible=False)
        # 2페이지 드랍때 전투판 밖으로 떨어지는 플레이어 체크하는 포탈 3페이지 시작할때 꼭 제거하기
        self.set_portal(portal_id=65, visible=False, enable=False, minimap_visible=False)
        # 2페이지 드랍때 전투판 밖으로 떨어지는 플레이어 체크하는 포탈 3페이지 시작할때 꼭 제거하기
        self.set_portal(portal_id=66, visible=False, enable=False, minimap_visible=False)
        # 무한 루프 빠지는 것을 막기 위해 이 변수 0 초기화 하기, 이후 이 변수 다시 사용할 일 없음
...
class 페이지_3PhaseWall_생성3(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(trigger_ids=[6701,6702,6703,6704,6705,6706,6707,6708,6709,6710,6711,6712,6713,6714,6715,6716,6717,6718,6719,6720,6721,6722,6723,6724,6725,6726,6727,6728,6729], visible=True, start_delay=1, interval=1, fade=1)
