...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[4220], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=4220, patrolName='MS2PatrolData_401')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=4220, patrolName='MS2PatrolData_402')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=4220, patrolName='MS2PatrolData_403')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9143, spawnIds=[4220]):
            return WaitGreeting()
...
class PickItem_30000695(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=4220, nifPath='Npc/Etc/UGC_SportsCar_Npc/UGC_SportsCar_Npc_02.nif', isOutline=True, scale=1.2, rotateZ=315)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=4220)
        set_conversation(type=1, spawnId=4220, script='$02000387_BF__4220_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=4220, patrolName='MS2PatrolData_444')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9304, spawnIds=[4220]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[4220])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=4220)
        set_conversation(type=1, spawnId=4220, script='$02000387_BF__4220_CUSTOMER__1$', arg4=3, arg5=0)
