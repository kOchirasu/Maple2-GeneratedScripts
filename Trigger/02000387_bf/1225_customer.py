...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[1225], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=1225, patrolName='MS2PatrolData_101')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=1225, patrolName='MS2PatrolData_102')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=1225, patrolName='MS2PatrolData_103')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9113, spawnIds=[1225]):
            return WaitGreeting()
...
class PickItem_30000693(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=1225, nifPath='Map/Kerningcity/Indoor/ke_in_prop_massage_A01.nif', isOutline=True, scale=1.2, rotateZ=225)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=1225)
        set_conversation(type=1, spawnId=1225, script='$02000387_BF__1225_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=1225, patrolName='MS2PatrolData_111')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9301, spawnIds=[1225]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[1225])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=1225)
        set_conversation(type=1, spawnId=1225, script='$02000387_BF__1225_CUSTOMER__1$', arg4=3, arg5=0)
