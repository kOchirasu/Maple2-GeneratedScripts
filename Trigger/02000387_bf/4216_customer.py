...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[4216], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=4216, patrolName='MS2PatrolData_401')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=4216, patrolName='MS2PatrolData_402')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=4216, patrolName='MS2PatrolData_403')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9143, spawnIds=[4216]):
            return WaitGreeting()
...
class PickItem_30000635(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=4216, nifPath='Map/Kerningcity/Indoor/ke_in_prop_tv_C01.nif', isOutline=True, scale=1.2, rotateZ=225)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=4216)
        set_conversation(type=1, spawnId=4216, script='$02000387_BF__4216_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=4216, patrolName='MS2PatrolData_444')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9304, spawnIds=[4216]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[4216])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=4216)
        set_conversation(type=1, spawnId=4216, script='$02000387_BF__4216_CUSTOMER__1$', arg4=3, arg5=0)
