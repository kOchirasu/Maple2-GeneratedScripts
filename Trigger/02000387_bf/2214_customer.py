...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[2214], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=2214, patrolName='MS2PatrolData_201')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=2214, patrolName='MS2PatrolData_202')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=2214, patrolName='MS2PatrolData_203')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9123, spawnIds=[2214]):
            return WaitGreeting()
...
class PickItem_30000701(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=2214, nifPath='Map/Common/Indoor/co_in_prop_security_A01.nif', isOutline=True, scale=1.2, rotateZ=225)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=2214)
        set_conversation(type=1, spawnId=2214, script='$02000387_BF__2214_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=2214, patrolName='MS2PatrolData_222')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9302, spawnIds=[2214]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[2214])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=2214)
        set_conversation(type=1, spawnId=2214, script='$02000387_BF__2214_CUSTOMER__1$', arg4=3, arg5=0)
