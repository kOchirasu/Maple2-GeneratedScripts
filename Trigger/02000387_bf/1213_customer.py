...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[1213], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=1213, patrolName='MS2PatrolData_101')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=1213, patrolName='MS2PatrolData_102')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=1213, patrolName='MS2PatrolData_103')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9113, spawnIds=[1213]):
            return WaitGreeting()
...
class PickItem_30000691(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=1213, nifPath='Map/Tria/Indoor/tr_in_prop_paintbag_A01.nif', isOutline=True, scale=1.2, rotateZ=225)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=1213)
        set_conversation(type=1, spawnId=1213, script='$02000387_BF__1213_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=1213, patrolName='MS2PatrolData_111')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9301, spawnIds=[1213]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[1213])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=1213)
        set_conversation(type=1, spawnId=1213, script='$02000387_BF__1213_CUSTOMER__1$', arg4=3, arg5=0)
