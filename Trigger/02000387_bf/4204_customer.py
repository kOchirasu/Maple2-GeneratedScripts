...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[4204], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=4204, patrolName='MS2PatrolData_401')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=4204, patrolName='MS2PatrolData_402')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=4204, patrolName='MS2PatrolData_403')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9143, spawnIds=[4204]):
            return WaitGreeting()
...
class PickItem_30000622(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=4204, nifPath='Map/Iceland/Indoor/ic_in_prop_snowball_A01.nif', isOutline=True, scale=1.2, rotateZ=225)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=4204)
        set_conversation(type=1, spawnId=4204, script='$02000387_BF__4204_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=4204, patrolName='MS2PatrolData_444')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9304, spawnIds=[4204]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[4204])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=4204)
        set_conversation(type=1, spawnId=4204, script='$02000387_BF__4204_CUSTOMER__1$', arg4=3, arg5=0)
