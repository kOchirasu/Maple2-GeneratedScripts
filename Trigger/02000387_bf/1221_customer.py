...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[1221], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=1221, patrolName='MS2PatrolData_101')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=1221, patrolName='MS2PatrolData_102')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=1221, patrolName='MS2PatrolData_103')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9113, spawnIds=[1221]):
            return WaitGreeting()
...
class PickItem_30000621(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=1221, nifPath='Map/Royalcity/Indoor/ry_in_prop_sandbag_A01.nif', isOutline=True, scale=1.2, rotateZ=225)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=1221)
        set_conversation(type=1, spawnId=1221, script='$02000387_BF__1221_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=1221, patrolName='MS2PatrolData_111')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9301, spawnIds=[1221]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[1221])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=1221)
        set_conversation(type=1, spawnId=1221, script='$02000387_BF__1221_CUSTOMER__1$', arg4=3, arg5=0)
