...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[3203], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=3203, patrolName='MS2PatrolData_301')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=3203, patrolName='MS2PatrolData_302')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=3203, patrolName='MS2PatrolData_303')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9133, spawnIds=[3203]):
            return WaitGreeting()
...
class PickItem_30000656(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=3203, nifPath='Map/SF/Indoor/sf_in_prop_bed_A01.nif', isOutline=True, scale=1.2, rotateZ=225)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=3203)
        set_conversation(type=1, spawnId=3203, script='$02000387_BF__3203_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=3203, patrolName='MS2PatrolData_333')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9303, spawnIds=[3203]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[3203])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=3203)
        set_conversation(type=1, spawnId=3203, script='$02000387_BF__3203_CUSTOMER__1$', arg4=3, arg5=0)
