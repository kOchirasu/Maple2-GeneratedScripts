...
class CustomerEnter(state.State):
    def on_enter(self):
        create_monster(spawnIds=[2206], arg2=False)
...
class Patrol01(state.State):
    def on_enter(self):
        move_npc(spawnId=2206, patrolName='MS2PatrolData_201')
...
class Patrol02(state.State):
    def on_enter(self):
        move_npc(spawnId=2206, patrolName='MS2PatrolData_202')
...
class Patrol03(state.State):
    def on_enter(self):
        move_npc(spawnId=2206, patrolName='MS2PatrolData_203')
...
class PatrolEnd(state.State):
    def on_tick(self) -> state.State:
        if npc_detected(boxId=9123, spawnIds=[2206]):
            return WaitGreeting()
...
class PickItem_30000682(state.State):
    def on_enter(self):
        ...
        add_effect_nif(spawnId=2206, nifPath='Map/Kerningcity/Indoor/ke_in_prop_display_E01.nif', isOutline=True, scale=1.2, rotateZ=225)
...
class RightItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=2206)
        set_conversation(type=1, spawnId=2206, script='$02000387_BF__2206_CUSTOMER__0$', arg4=3, arg5=0)
...
class CustomerLeave(state.State):
    def on_enter(self):
        move_npc(spawnId=2206, patrolName='MS2PatrolData_222')

    def on_tick(self) -> state.State:
        if npc_detected(boxId=9302, spawnIds=[2206]):
            return Quit()


class Quit(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[2206])
...
class WrongItem(state.State):
    def on_enter(self):
        ...
        remove_effect_nif(spawnId=2206)
        set_conversation(type=1, spawnId=2206, script='$02000387_BF__2206_CUSTOMER__1$', arg4=3, arg5=0)
