...
class 전투감지(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[101,111,201,211,301,311,401,411]):
            return 전투감지2(self.ctx)


class 전투감지2(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[102,202,302,402]):
