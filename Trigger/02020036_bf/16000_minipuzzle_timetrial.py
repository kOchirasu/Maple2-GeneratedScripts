...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[12000263], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001161 걸어서 71001061 제거
...
class TimeTrial_Success(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[12000263], state=1) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001161 걸어서 71001061 제거

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[12000263], arg2=0):
            set_interact_object(triggerIds=[12000263], state=2)
            set_user_value(triggerId=16000, key='TimeEventOn', value=0)
            return TimeTrial_SuccessDelay()
        if time_expired(timerId='100'):
            set_interact_object(triggerIds=[12000263], state=0)
            reset_timer(timerId='100')
