...
class Wait(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000263], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001161 걸어서 71001061 제거
...
class TimeTrial_Success(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000263], state=1) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001161 걸어서 71001061 제거

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[12000263], stateValue=0):
            self.set_interact_object(triggerIds=[12000263], state=2)
        ...
        if self.time_expired(timerId='100'):
            self.set_interact_object(triggerIds=[12000263], state=0)
            self.reset_timer(timerId='100')
