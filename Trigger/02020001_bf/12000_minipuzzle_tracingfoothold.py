...
class Wait(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000221], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001121 걸어서 71001021 제거
...
class TracingFootHold_Success(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000221], state=1) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001121 걸어서 71001021 제거
        self.set_timer(timerId='2', seconds=60, startDelay=1, interval=0, vOffset=0)

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[12000221], stateValue=0):
            return TracingFootHold_SuccessDelay(self.ctx)
        if self.time_expired(timerId='2'):
            self.set_interact_object(triggerIds=[12000221], state=2)
