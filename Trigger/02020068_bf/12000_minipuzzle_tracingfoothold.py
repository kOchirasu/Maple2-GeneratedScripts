...
class Wait(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000225], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001121 걸어서 71001021 제거
...
class TracingFootHold_Start_Delay(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1000):
            return TracingFootHold_Play01(self.ctx)


class TracingFootHold_Play01(common.Trigger):
    def on_enter(self):
...
class TracingFootHold_Play01_Delay(common.Trigger):
    def on_enter(self):
...
class TracingFootHold_Play02(common.Trigger):
    def on_enter(self):
...
class TracingFootHold_Play02_Delay(common.Trigger):
    def on_enter(self):
...
# 퍼즐 성공 후 종료
class TracingFootHold_Success(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000225], state=1) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001121 걸어서 71001021 제거
        self.set_timer(timerId='2', seconds=60, startDelay=1, interval=0, vOffset=0)

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[12000225], stateValue=0):
            return TracingFootHold_SuccessDelay(self.ctx)
        if self.time_expired(timerId='2'):
            self.set_interact_object(triggerIds=[12000225], state=2)
            return ResetTimer(self.ctx)


class TracingFootHold_SuccessDelay(common.Trigger):
    def on_enter(self):
...
# 제한 시간 종료
class TracingFootHold_Fail(common.Trigger):
    def on_enter(self):
        self.set_actor(triggerId=12201, visible=False, initialSequence='Interaction_luminous_A01_on') # Actor_FlowerOn01
        self.set_actor(triggerId=12202, visible=False, initialSequence='Interaction_luminous_A01_on') # Actor_FlowerOn02
