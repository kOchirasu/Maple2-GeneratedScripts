""" trigger/52010054_qd/52010054_qd.xml """
from common import *
import state


class start(state.State):
    def on_enter(self):
        visible_my_pc(isVisible=False) # 유저 투명 처리
        create_monster(spawnIds=[2000], arg2=False) # 인페르녹
        create_monster(spawnIds=[2001], arg2=False) # 크림슨 발록
        create_monster(spawnIds=[2002], arg2=False) # 크림슨 발록
        create_monster(spawnIds=[2003], arg2=False) # 크림슨 발록

    def on_tick(self) -> state.State:
        if check_user():
            return CameraEffect01()


class CameraEffect01(state.State):
    def on_enter(self):
        set_onetime_effect(id=1, enable=True, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')
        set_cinematic_ui(type=1)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=1000):
            return CameraEffect02()


class CameraEffect02(state.State):
    def on_enter(self):
        set_scene_skip(state=quit02, arg2='nextState')
        set_onetime_effect(id=1, enable=False, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')
        select_camera_path(pathIds=[4000,4001], returnView=False)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=14000):
            return CameraEffect03()


class CameraEffect03(state.State):
    def on_enter(self):
        set_onetime_effect(id=2, enable=True, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=3000):
            return CameraEffect04()


class CameraEffect04(state.State):
    def on_enter(self):
        set_onetime_effect(id=2, enable=False, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')
        add_cinematic_talk(npcId=11003793, msg='$52010054_QD__52010054_QD__0$', duration=7000, align='right')
        select_camera_path(pathIds=[4002,4003], returnView=False)
        set_cinematic_ui(type=3)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=10000):
            return CameraEffect05()


class CameraEffect05(state.State):
    def on_enter(self):
        add_cinematic_talk(npcId=11003793, msg='$52010054_QD__52010054_QD__1$', duration=5000, align='right')
        select_camera_path(pathIds=[4004,4005], returnView=False)
        set_npc_emotion_sequence(spawnId=2001, sequenceName='Attack_01_C,Attack_Idle_A')

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=5000):
            return CameraEffect06()


class CameraEffect06(state.State):
    def on_enter(self):
        add_cinematic_talk(npcId=11003793, msg='$52010054_QD__52010054_QD__2$', duration=4000, align='right')
        select_camera_path(pathIds=[4006,4007], returnView=False)
        set_npc_emotion_sequence(spawnId=2002, sequenceName='Attack_01_B,Attack_Idle_A')

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=5000):
            return CameraEffect07()


class CameraEffect07(state.State):
    def on_enter(self):
        add_cinematic_talk(npcId=11003793, msg='$52010054_QD__52010054_QD__3$', duration=4000, align='right')
        select_camera_path(pathIds=[4008,4009,4013,4014], returnView=False)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=10000):
            return CameraEffect08()


class CameraEffect08(state.State):
    def on_enter(self):
        set_onetime_effect(id=3, enable=True, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=3000):
            return CameraEffect09()


class CameraEffect09(state.State):
    def on_enter(self):
        set_cinematic_ui(type=2)
        destroy_monster(spawnIds=[2001], arg2=False) # 크림슨 발록
        destroy_monster(spawnIds=[2002], arg2=False) # 크림슨 발록
        destroy_monster(spawnIds=[2003], arg2=False) # 크림슨 발록

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=0):
            return CameraEffect10()


class CameraEffect10(state.State):
    def on_enter(self):
        set_onetime_effect(id=3, enable=False, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')
        set_cinematic_ui(type=1)
        select_camera_path(pathIds=[4010,4011], returnView=False)
        set_time_scale(enable=True, startScale=1, endScale=0.1, duration=12, interpolator=1) # 10초간 느려지기 시작

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=12000):
            return CameraEffect11()


class CameraEffect11(state.State):
    def on_enter(self):
        set_onetime_effect(id=4, enable=True, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=200):
            return CameraEffect12()


class CameraEffect12(state.State):
    def on_enter(self):
        set_onetime_effect(id=4, enable=False, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=200):
            return CameraEffect13()


class CameraEffect13(state.State):
    def on_enter(self):
        set_npc_emotion_sequence(spawnId=2000, sequenceName='Attack_01_B,Attack_01_B')
        set_time_scale(enable=True, startScale=0.5, endScale=0.1, duration=12, interpolator=1) # 10초간 느려지기 시작
        select_camera_path(pathIds=[4012,4015], returnView=False)

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=6000):
            return quit01()


class quit01(state.State):
    def on_enter(self):
        set_onetime_effect(id=5, enable=True, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=2000):
            return quit02()


class quit02(state.State):
    def on_enter(self):
        set_onetime_effect(id=5, enable=False, path='BG/Common/ScreenMask/Eff_CameraMasking_FastFadeIn.xml')
        move_user(mapId=2000422, portalId=5)


