...
class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(triggerIds=[12000108], state=2)
...
class 레버2_발동(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_interact_object(triggerIds=[12000108], state=1)
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.monster_dead(boxIds=[701]):
            return 종료(self.ctx)
        if self.object_interacted(interactIds=[12000108], stateValue=0):
...
class 레버2_재활성(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='ObjectStart', value=2) and self.monster_dead(boxIds=[701]):
            return 종료(self.ctx)
        if self.object_interacted(interactIds=[12000108], stateValue=0):
