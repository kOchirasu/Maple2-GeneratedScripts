...
class 카메라_아노스줌인(common.Trigger):
    def on_enter(self):
...
class 카메라_아노스줌인01(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=2000):
            return 카메라_케이틀린01(self.ctx)


class 카메라_케이틀린01(common.Trigger):
    def on_enter(self):
...
class 카메라_케이틀린0102(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1000):
            return 카메라_케이틀린02(self.ctx)


class 카메라_케이틀린02(common.Trigger):
    def on_enter(self):
...
class 아노스아파_스킵완료(common.Trigger):
    def on_enter(self):
        self.set_cinematic_ui(type=1)
