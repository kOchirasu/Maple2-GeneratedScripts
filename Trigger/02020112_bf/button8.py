...
class 작동(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='ButtonSuccess', value=1):
            return 종료(self.ctx)
        if self.user_detected(boxIds=[924], jobCode=0):
            return 감지(self.ctx)


class 감지(trigger_api.Trigger):
    def on_enter(self):
        self.set_actor(triggerId=9908, visible=True, initialSequence='Interaction_Lapentafoothold_A01_On')

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='ButtonSuccess', value=1):
            return 종료(self.ctx)
        if not self.user_detected(boxIds=[924], jobCode=0):
            return 작동(self.ctx)
