...
# 두 번째 퀘스트 수락한 상태
class QuestOnGoing04(common.Trigger):
    def on_enter(self):
        self.move_user(mapId=63000024, portalId=10, boxId=9900)
        self.set_cinematic_ui(type=0)
        self.set_cinematic_ui(type=2)
...
class TinChaiTalk04(common.Trigger):
    def on_enter(self):
        self.remove_cinematic_talk()
        self.move_user(mapId=63000024, portalId=10, boxId=9900)
        self.set_cinematic_ui(type=0)
...
class TinChaiTalk04_CSkip(common.Trigger):
    def on_enter(self):
        self.remove_cinematic_talk()
        self.move_npc(spawnId=101, patrolName='MS2PatrolData_102')
        self.move_user(mapId=63000024, portalId=10, boxId=9900)
...
class MeetTinChai01(common.Trigger):
    def on_enter(self):
...
class MeetTinChai02(common.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[5000], visible=False) # 가이드 서머리 사운드 이펙트
        self.set_effect(triggerIds=[5001], visible=False) # 화살표 안내 사운드 이펙트
...
class TinChaiTalk10(common.Trigger):
    def on_enter(self):
        self.set_cinematic_ui(type=1)
        self.set_cinematic_ui(type=3)
...
class TinChaiTalk11(common.Trigger):
    def on_enter(self):
...
class TinChaiTalk12(common.Trigger):
    def on_enter(self):
        self.remove_cinematic_talk()
        self.set_skip()
...
class TinChaiTalk13(common.Trigger):
    def on_enter(self):
...
class TinChaiTalk14(common.Trigger):
    def on_enter(self):
        self.remove_cinematic_talk()
        self.set_skip()
        self.set_cinematic_ui(type=0)
...
class QuestStart01(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.quest_user_detected(boxIds=[9900], questIds=[90000443], questStates=[2]):
            return QuestStart02(self.ctx)


class QuestStart02(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.quest_user_detected(boxIds=[9900], questIds=[90000443], questStates=[3]):
            return QuestStart03(self.ctx)
...
# 약초 장착 가이드
class QuestStart03(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.quest_user_detected(boxIds=[9900], questIds=[90000444], questStates=[1]):
            return QuestStart04(self.ctx)


class QuestStart04(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=1, visible=True, enable=False, minimapVisible=False)
...
class GuideNextMap03(common.Trigger):
    def on_enter(self):
