...
class 영상딜레이(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_timer(timerId='101', seconds=30)
...
class 움직이기01(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.set_effect(triggerIds=[5060], visible=False)
        # 이슈라 음성 사운드 이펙트 01
...
class 움직이기02(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.set_effect(triggerIds=[5051], visible=True)
        # 미션 완료 사운드 이펙트
...
class 이동완료01(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.set_effect(triggerIds=[5050], visible=False)
        # 가이드 서머리 사운드 이펙트
...
class 미니맵가이드01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.guide_event(eventId=10010010)
...
class 다리붕괴01(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        ...
        self.set_effect(triggerIds=[5050], visible=False)
        # 가이드 서머리 사운드 이펙트
        self.set_effect(triggerIds=[5052], visible=False)
        # 화살표  안내 사운드 이펙트
...
class 이슈라교체(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        self.set_effect(triggerIds=[5066], visible=False)
        # 이슈라 음성 사운드 이펙트 07
...
class 퀘스트수락유도01(trigger_api.Trigger):
    ...
    def on_exit(self) -> None:
        ...
        self.set_effect(triggerIds=[5040], visible=False)
        # toward portal 01
        self.set_effect(triggerIds=[5041], visible=False)
        # toward portal 02
        self.set_effect(triggerIds=[5042], visible=False)
        # toward portal 03
        self.set_effect(triggerIds=[5043], visible=False)
        # toward portal 04
        self.set_effect(triggerIds=[5044], visible=False)
        # toward portal 05
        self.set_effect(triggerIds=[5045], visible=False)
        # toward portal 06
        self.set_effect(triggerIds=[5046], visible=False)
        # toward portal 07
        self.set_effect(triggerIds=[5020], visible=False)
        # after swim 01
        self.set_effect(triggerIds=[5021], visible=False)
        # after swim 02
        self.set_effect(triggerIds=[5022], visible=False)
        # after swim 03
        self.set_effect(triggerIds=[5023], visible=False)
        # after swim 04
        self.set_effect(triggerIds=[5024], visible=False)
        # after swim 05
        self.set_effect(triggerIds=[5025], visible=False)
        # after swim 06
        self.set_effect(triggerIds=[5026], visible=False)
        # after swim 07
        self.set_effect(triggerIds=[5027], visible=False)
        # after swim 08
        self.set_effect(triggerIds=[5028], visible=False)
        # after swim 09
...
class 포털생성01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.guide_event(eventId=10010080)
