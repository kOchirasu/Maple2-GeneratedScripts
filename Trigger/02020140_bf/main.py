...
class 시작대기중(common.Trigger):
    def on_enter(self):
        ...
        self.set_portal(portalId=598, visible=False, enable=False, minimapVisible=False) # 최초 시작 지점에 바로 2페이지 메인 전투판으로 포내는 포탈 처음에는 감추기, 참고로 포탈 arg1="598" 과 arg1="108"은 스타팅 포인트 같은 좌표에 있음
...
class 페이즈보스등장대기1(common.Trigger):
    def on_tick(self) -> common.Trigger:
...
class 페이즈전투완료_2페이즈투르카등장1(common.Trigger):
    def on_enter(self):
...
class 졸구간시작연출딜레이(common.Trigger):
    def on_enter(self):
        ...
        self.change_background(dds='BG_Turka_D.dds') # 2페이즈 부터 맵이 밝아지면서, 백그라운드 이미지 교체함
...
class 최초졸구간몬스터등장(common.Trigger):
    def on_enter(self):
        ...
        self.create_monster(spawnIds=[1101,1102,1103,1104,1105,1106,1107,1108,1109,1110,1111,1112,1113,1114,1115], animationEffect=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
        self.create_monster(spawnIds=[1201,1202,1203,1204,1205], animationEffect=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
        self.create_monster(spawnIds=[1301,1302,1303,1304,1305,1306,1307,1308,1309,1310,1311,1312,1313], animationEffect=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 두번째왼쪽진행01(common.Trigger):
    def on_enter(self):
        ...
        self.create_monster(spawnIds=[2101,2102,2103,2104,2105,2106,2107,2108,2109,2110,2111,2112,2113,2114,2115,2116,2117,2118,2119,2120], animationEffect=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 두번째왼쪽진행02(common.Trigger):
    def on_enter(self):
...
class 두번째가운데진행01(common.Trigger):
    def on_enter(self):
        ...
        self.create_monster(spawnIds=[2201,2202,2203,2204,2205], animationEffect=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 두번째가운데진행02(common.Trigger):
    def on_enter(self):
...
class 세번째가운데진행01(common.Trigger):
    def on_enter(self):
        ...
        self.create_monster(spawnIds=[3201,3202,3203,3204,3205], animationEffect=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 세번째가운데진행02(common.Trigger):
    def on_enter(self):
...
class 두번째오른쪽진행01(common.Trigger):
    def on_enter(self):
        ...
        self.create_monster(spawnIds=[2301,2302,2303,2304,2305,2306,2307,2308,2309,2310,2311,2312,2313,2314,2315], animationEffect=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 두번째오른쪽진행02(common.Trigger):
    def on_enter(self):
...
class 왼쪽지점보스방가는포탈생성(common.Trigger):
    def on_enter(self):
...
class 가운데지점보스방가는포탈생성(common.Trigger):
    def on_enter(self):
...
class 오른쪽지점보스방가는포탈생성(common.Trigger):
    def on_enter(self):
...
class 모든졸구간지형과포탈생성(common.Trigger):
    def on_enter(self):
...
class 이맵에서진행끝내고다음맵으로이동(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=599, visible=True, enable=True, minimapVisible=True) # 세번째페이즈 진행하는 다음 맵으로 이동시키기, 2페이지 전투판에 위치한 포탈
        self.set_portal(portalId=598, visible=True, enable=True, minimapVisible=True) # 최초 시작 지점에 바로 2페이지 메인 전투판으로 포내는 포탈 생성하기, 참고로 포탈 arg1="598" 과 arg1="108"은 스타팅 포인트 같은 좌표에 있음
        ...
        self.set_portal(portalId=428, visible=True, enable=True, minimapVisible=True) # 보스 전투판으로 가는 가운데 졸구간 포탈 생성
        self.set_portal(portalId=338, visible=True, enable=True, minimapVisible=True) # 보스 전투판으로 가는 오른쪽 졸구간 포탈 생성
        self.set_sound(triggerId=140140, enable=True) # 이 맵에서 전투 끝났으니, 보스BGM끄고 일반BGM이 나오게 하기,  TimeCheck.xml 트리거에서도 같은 BGM 교체 트리거 사용함
        self.destroy_monster(spawnIds=[1101,1102,1103,1104,1105,1106,1107,1108,1109,1110,1111,1112,1113,1114,1115,2101,2102,2103,2104,2105,2106,2107,2108,2109,2110,2111,2112,2113,2114,2115,2116,2117,2118,2119,2120])
        self.destroy_monster(spawnIds=[1201,1202,1203,1204,1205,2201,2202,2203,2204,2205,3201,3202,3203,3204,3205])
        self.destroy_monster(spawnIds=[1301,1302,1303,1304,1305,1306,1307,1308,1309,1310,1311,1312,1313,2301,2302,2303,2304,2305,2306,2307,2308,2309,2310,2311,2312,2313,2314,2315])
...
class 종료안내메시지_대기(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_detected(boxIds=[97]):
            return 종료안내메시지(self.ctx)


class 종료안내메시지(common.Trigger):
    def on_enter(self):
...
class 종료안내메시지_다시출력_대기(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_detected(boxIds=[97]):
            return 종료안내메시지_다시출력(self.ctx)


class 종료안내메시지_다시출력(common.Trigger):
    def on_enter(self):
        self.show_guide_summary(entityId=29200004, textId=29200004)
