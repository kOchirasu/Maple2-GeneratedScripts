...
class 단계_튜브대미지필드_TubeLeveStep_더하기2(state.State):
    ...
    def on_tick(self) -> state.State:
        ...
        if user_value(key='TubeLeveStep', value=30):
            return 버프부여구슬제거경고메시지()
...
class 버프부여구슬제거경고메시지(state.State):
    def on_enter(self):
        add_user_value(key='TubeLeveStep', value=-9) # 이 변수 -9를 빼서 21로 만들기, 절대 20 이하로 만들면 안됨!! 대박 버그임
        add_buff(boxIds=[102], skillId=50000348, level=2, arg4=True, arg5=True) # 벙업 버프 (5중첩) 걸기(50000348 2Lv), 홀로그램 소녀 소환몹 몬스터가 보스한테 부여하는 버프랑 동일한 것임
        show_guide_summary(entityId=29200005, textId=29200005, duration=5000)
...
class 단계_튜브대미지필드_제거2(state.State):
    def on_enter(self):
        ...
        npc_remove_additional_effect(spawnId=102, additionalEffectId=50000348)
        ...
        hide_guide_summary(entityId=29200005) # 혹시 마력의 구슬 제거하라는 경고 메시지 떠있는 상태일 경우를 대비해 메시지 제거 명령어 여기에 설정함
