...
class 대기(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.count_users(boxId=1001008, minUsers='1', operator='Equal'):
...
class 버프발동(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(waitTick=100):
            return 버프발동(self.ctx)
        if self.count_users(boxId=1001008, minUsers='1', operator='Greater'):
            return 대기(self.ctx)
        if not self.user_detected(boxIds=[1001008]):
            return 대기(self.ctx)
