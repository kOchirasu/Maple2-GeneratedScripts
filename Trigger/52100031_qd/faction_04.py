...
class 대기(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10002066], state=2)
        set_interact_object(triggerIds=[10002070], state=2)
...
class 인원수체크(state.State):
    def on_enter(self):
        ...
        set_conversation(type=1, spawnId=1301, script='$52100031_QD__FACTION_04__0$', arg4=5, arg5=0)
...
class 퀘스트(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002070], state=1)
...
class 보스소환(state.State):
    ...
    def on_tick(self) -> state.State:
        if user_value(key='DungeonClear', value=1):
            ...
            set_interact_object(triggerIds=[10002066], state=0)
            set_interact_object(triggerIds=[10002070], state=0)
            remove_buff(boxId=199, skillId=99910160)
            return 종료()
