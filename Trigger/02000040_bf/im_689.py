...
class 시작대기중(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10000689], state=1)
...
class 오브젝트반응(state.State):
    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10000689], arg2=0):
            return 시간텀()
...
class NPC이동(state.State):
    def on_enter(self):
        ...
        set_conversation(type=1, spawnId=1104, script='$02000040_BF__IM_689__0$', arg4=2)
