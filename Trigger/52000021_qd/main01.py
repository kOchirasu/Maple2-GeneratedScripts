...
class 주문석반응대기(common.Trigger):
    ...
    def on_exit(self):
        self.set_interact_object(triggerIds=[10000831], state=2) # 보호 룬 주문석 : Shield ON
        self.set_interact_object(triggerIds=[10000832], state=0) # 보호 룬 주문석 : Shield OFF
        self.set_effect(triggerIds=[6000], visible=True) # 보호막 이펙트
        self.set_mesh(triggerIds=[4000,4001,4002,4003], visible=True, arg3=0, delay=0, scale=0) # 보호막 영역 탈출 제한


# 유저가 영역 안에 들어온 상태
class 보호막설정01(common.Trigger):
    def on_enter(self):
...
class 유저감지01(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_detected(boxIds=[9001]):
            return 탈출경고01(self.ctx)


class 탈출경고01(common.Trigger):
    def on_enter(self):
