...
class 스폰대사1(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(boxIds=[101]):
            self.add_balloon_talk(spawnId=101, msg='너 따위 애송이에게 지다니...', duration=3000)
...
class 스폰대사2(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(boxIds=[102]):
            self.add_balloon_talk(spawnId=102, msg='꽤 하는군. 즐거운 싸움이었다.', duration=3000)
...
class 스폰대사3(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(boxIds=[103]):
            self.add_balloon_talk(spawnId=103, msg='좋은 실력입니다. 엘리넬 학생들에게 귀감이 될 분이시군요.', duration=3000)
            return 대사(self.ctx)
