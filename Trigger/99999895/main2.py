...
class 스폰대사1(state.State):
    def on_enter(self):
        add_balloon_talk(spawnId=201, msg='네 마음가짐과 기량을 한 번 볼까?', duration=3000)

    def on_tick(self) -> state.State:
        if monster_dead(boxIds=[201]):
            add_balloon_talk(spawnId=201, msg='너 따위 애송이에게 지다니...', duration=3000)
            return 몬스터스폰대기2()
...
class 스폰대사2(state.State):
    def on_enter(self):
        add_balloon_talk(spawnId=202, msg='와라!! 얼마나 성장했는지 보겠다!', duration=3000)

    def on_tick(self) -> state.State:
        if monster_dead(boxIds=[202]):
            add_balloon_talk(spawnId=202, msg='꽤 하는군. 즐거운 싸움이었다.', duration=3000)
            return 몬스터스폰대기3()
...
class 스폰대사3(state.State):
    def on_enter(self):
        add_balloon_talk(spawnId=203, msg='그동안 어떤 성과를 이루셨는지 보여주세요.', duration=3000)

    def on_tick(self) -> state.State:
        if monster_dead(boxIds=[203]):
            add_balloon_talk(spawnId=203, msg='좋은 실력입니다. 엘리넬 학생들에게 귀감이 될 분이시군요.', duration=3000)
            return 대사()
