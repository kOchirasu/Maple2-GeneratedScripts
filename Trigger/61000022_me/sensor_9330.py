...
class Wait(state.State):
    def on_enter(self):
        set_user_value(key='Box33Check', value=10)
        set_mesh(triggerIds=[533], visible=True, arg3=0, arg4=0, arg5=0) # 33 / Ground outter
        set_mesh(triggerIds=[5330], visible=True, arg3=0, arg4=0, arg5=0) # 33 / Ground inner

    def on_tick(self) -> state.State:
        if user_value(key='Box33Check', value=0):
            return Sensor0()
        if user_value(key='Box33Check', value=1):
            return Sensor1()
        if user_value(key='Box33Check', value=2):
            return Sensor2()
        if user_value(key='Box33Check', value=3):
            return Sensor3()
        if user_value(key='Box33Check', value=4):
            return Sensor4()
        if user_value(key='Box33Check', value=5):
            return Sensor5()
        if user_value(key='Box33Check', value=7):
            return Sensor7()
        if user_value(key='Box33Check', value=8):
            return Sensor8()
        if user_value(key='Box33Check', value=9):
            return Sensor9()
        if user_value(key='Box33Check', value=6):
            return Sensor10()
        if user_value(key='Box33Check', value=15):
            return Sensor15()
        if user_value(key='Box33Check', value=20):
            return Sensor20()
        if user_value(key='Box33Check', value=25):
            return Sensor25()
        if user_value(key='Box33Check', value=30):
            return Sensor30()
...
class NormalPass(state.State):
    def on_enter(self):
        play_system_sound_in_box(boxIds=[9330], sound='DDStop_Stage_Pass_01')
        set_user_value(triggerId=7330, key='Color33', value=0) # color reset
        set_mesh(triggerIds=[533], visible=False, arg3=0, arg4=0, arg5=2) # 33 / Ground outter
...
class GamblePass(state.State):
    def on_enter(self):
        play_system_sound_in_box(boxIds=[9330], sound='DDStop_Stage_Pass_01')
        set_user_value(triggerId=7330, key='Color33', value=0) # color reset
        set_user_value(triggerId=3, key='GamblePass', value=33) # Gamble Pass Bonus For Everyone
        set_mesh(triggerIds=[533], visible=False, arg3=0, arg4=0, arg5=2) # 33 / Ground outter
        write_log(logName='dancedancestop', event='9330', arg3='char_event', arg4='4', arg5='gamble')
...
class JackpotPass(state.State):
    def on_enter(self):
        play_system_sound_in_box(boxIds=[9330], sound='DDStop_Stage_Pass_01')
        set_user_value(triggerId=7330, key='Color33', value=0) # color reset
        set_user_value(triggerId=3, key='JackpotPass', value=33) # Jackpot Pass Bonus For Everyone
        set_mesh(triggerIds=[533], visible=False, arg3=0, arg4=0, arg5=2) # 33 / Ground outter
        write_log(logName='dancedancestop', event='9330', arg3='char_event', arg4='4', arg5='jackpot')
...
class Fail(state.State):
    def on_enter(self):
        play_system_sound_in_box(boxIds=[9330], sound='DDStop_Stage_Fail_01')
        set_mesh(triggerIds=[533], visible=False, arg3=0, arg4=0, arg5=2) # 33 / Ground outter
        set_mesh(triggerIds=[5330], visible=False, arg3=0, arg4=0, arg5=0) # 33 / Ground inner
        set_user_value(triggerId=7330, key='Color33', value=4) # color clear
...
class GambleReset(state.State):
    def on_enter(self):
        set_achievement(triggerId=9330, type='trigger', achieve='ddstop_gamble')
        mini_game_give_exp(boxId=9330, expRate=0.1, isOutside=False) # Gamble Pass Bonus For challenger
        set_user_value(key='Box33Check', value=10)
...
class JackpotReset(state.State):
    def on_enter(self):
        set_achievement(triggerId=9330, type='trigger', achieve='ddstop_gamble')
        mini_game_give_exp(boxId=9330, expRate=0.3, isOutside=False) # Jackpot Pass Bonus For challenger
        set_user_value(key='Box33Check', value=10)
