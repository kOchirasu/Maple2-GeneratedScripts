...
class 트리거영역체크시작(common.Trigger):
    def on_tick(self) -> common.Trigger:
        ...
        if self.monster_dead(boxIds=[99]): # ##  보스가 죽으면 졸몹 등장 트리거 종료시키기 ##
            return 졸몬스터제거작업(self.ctx)
        if self.user_detected(boxIds=[11000]):
...
class 트리거영역안플레이어최종체크(common.Trigger):
    def on_tick(self) -> common.Trigger:
        ...
        if self.monster_dead(boxIds=[99]): # ##  보스가 죽으면 졸몹 등장 트리거 종료시키기 ##
            return 졸몬스터제거작업(self.ctx)
        if self.user_detected(boxIds=[11000]): # waitTick 후에도 플레이어가 트리거 박스 안에서 벗어났으면, 다시 처음단계로 돌아가기
...
class 졸몬스터등장하기(common.Trigger):
    def on_enter(self):
        self.create_monster(spawnIds=[11001,11002,11003,11004], animationEffect=False)
...
class 트리거영역에계속있는지체크(common.Trigger):
    def on_tick(self) -> common.Trigger:
        ...
        if not self.user_detected(boxIds=[11000]):
...
class 졸몬스터제거작업(common.Trigger):
    def on_enter(self):
        self.destroy_monster(spawnIds=[11001,11002,11003,11004])
