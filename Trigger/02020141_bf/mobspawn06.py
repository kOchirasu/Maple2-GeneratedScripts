...
class 트리거영역안플레이어최종체크(state.State):
    def on_tick(self) -> state.State:
        ...
        if monster_dead(boxIds=[99]): # ##  보스가 죽으면 졸몹 등장 트리거 종료시키기 ##
            return 졸몬스터제거작업()
        if user_detected(boxIds=[10600]): # waitTick 후에도 플레이어가 트리거 박스 안에서 벗어났으면, 다시 처음단계로 돌아가기
            return 졸몬스터등장하기()
...
class 졸몬스터등장하기(state.State):
    def on_enter(self):
        create_monster(spawnIds=[10601,10602,10603,10604], arg2=False)
...
class 트리거영역에계속있는지체크(state.State):
    def on_tick(self) -> state.State:
        ...
        if user_detected(boxIds=[10600]):
            return 졸몬스터리젠단계시작()
        if not user_detected(boxIds=[10600]):
            return 졸몬스터제거작동대기()
...
class 졸몬스터제거작업(state.State):
    def on_enter(self):
        destroy_monster(spawnIds=[10601,10602,10603,10604])
