...
class 레논등장(common.Trigger):
    def on_enter(self):
        self.set_onetime_effect(id=1, enable=True, path='BG\weather\Eff_monochrome_03.xml') # 연출 3 : 레터박스 
			연출 1 : 플레이어 조작 뺏기
...
class 카메라시점변경_ready(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=2000):
            return 카메라시점변경(self.ctx)


class 카메라시점변경(common.Trigger):
    def on_enter(self):
