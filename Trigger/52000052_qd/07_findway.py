...
class ReadyToWalkIn01(state.State):
    def on_enter(self):
        ...
        move_npc(spawnId=102, patrolName='MS2PatrolData_107')
        move_npc(spawnId=202, patrolName='MS2PatrolData_207')
...
class Round07_Start(state.State):
    def on_enter(self):
        ...
        set_conversation(type=1, spawnId=1007, script='$52000052_QD__04_FINDWAY__2$', arg4=3, arg5=2) # 틴차이
...
class Round07_Sucess02(state.State):
    def on_enter(self):
        ...
        set_conversation(type=1, spawnId=107, script='$52000052_QD__04_FINDWAY__3$', arg4=2, arg5=1) # 틴차이
...
class Round07_PickRoute_Left(state.State):
    def on_enter(self):
        set_user_value(triggerId=1307, key='MakeTrue', value=1)
        set_user_value(triggerId=2307, key='MakeFalse', value=1)
...
class Round07_PickRoute_Right(state.State):
    def on_enter(self):
        set_user_value(triggerId=1307, key='MakeFalse', value=1)
        set_user_value(triggerId=2307, key='MakeTrue', value=1)
...
class GoToRound10(state.State):
    def on_enter(self):
        set_user_value(triggerId=10, key='FindWayLeft', value=1)
