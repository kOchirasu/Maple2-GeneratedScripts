...
class 시작대기중(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10000293], state=1)
        set_actor(triggerId=201, visible=True, initialSequence='Dead_A')

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10000293], arg2=0):
            return 오브젝트반응()
...
class NPC이동(state.State):
    def on_enter(self):
        move_npc(spawnId=301, patrolName='MS2PatrolData_201')
        set_conversation(type=1, spawnId=301, script='$02000046_AD__EAGLE_01__0$', arg4=2)
