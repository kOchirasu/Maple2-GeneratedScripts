...
class 타임머신중지(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_detected(boxIds=[104]):
            return 보스방이동준비(self.ctx)


class 보스방이동준비(common.Trigger):
    def on_enter(self):
