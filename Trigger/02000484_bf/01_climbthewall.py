...
class Wait(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[10002030], state=0) # Lever
        self.set_interact_object(triggerIds=[10002031], state=0) # Lever
...
class FirstArrowGuide01(common.Trigger):
    def on_enter(self):
...
class TerraceMobAttack01(common.Trigger):
    def on_enter(self):
        self.create_monster(spawnIds=[901,902,903], animationEffect=False) # Mob_Actor
        self.set_conversation(type=1, spawnId=901, script='$02000484_BF__01_CLIMBTHEWALL__0$', arg4=2, arg5=1)
        self.set_conversation(type=1, spawnId=902, script='$02000484_BF__01_CLIMBTHEWALL__1$', arg4=2, arg5=1)
        self.set_conversation(type=1, spawnId=903, script='$02000484_BF__01_CLIMBTHEWALL__2$', arg4=2, arg5=1)
...
class TerraceMobAttack02(common.Trigger):
    def on_enter(self):
...
class LiftUpBombGuide01(common.Trigger):
    def on_enter(self):
        self.play_system_sound_in_box(sound='System_ShowGuideSummary_01')
        self.show_guide_summary(entityId=20039802, textId=20039802, duration=4000) # 가이드 : 폭탄을 들어서 가로막힌 벽을 향해 던지기
...
class SecondArrowGuide01(common.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[5500,5501,5502,5503,5504], visible=False) # ArrowGuide04
...
class TowerUnderMobAttack01(common.Trigger):
    def on_enter(self):
        self.play_system_sound_in_box(sound='System_ShowGuideSummary_01')
        self.show_guide_summary(entityId=20039805, textId=20039805, duration=3000) # 가이드 : 레버를 당기기
...
class TowerUnderMobAttack01End(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[920,921,922,923]):
            return SwichOn(self.ctx)


class SwichOn(common.Trigger):
    def on_enter(self):
        self.set_interact_object(triggerIds=[10002030], state=1) # LeverForLadder01

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[10002030], stateValue=0):
            return LadderOnToTowerUp01(self.ctx)


class LadderOnToTowerUp01(common.Trigger):
    def on_enter(self):
        self.play_system_sound_in_box(sound='System_ShowGuideSummary_01')
...
class GuideMoveToBridge01(common.Trigger):
    def on_enter(self):
...
class GuideMoveToBridge01_Guide(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_detected(boxIds=[9350]):
            return GuideMoveToBridge01End(self.ctx)


class GuideMoveToBridge01End(common.Trigger):
    def on_enter(self):
        self.show_guide_summary(entityId=20039805, textId=20039805, duration=3000) # 가이드 : 레버를 당기기

    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[930,931,932,933,934]):
            return GuidePullTheLever01(self.ctx)


class GuidePullTheLever01(common.Trigger):
    def on_enter(self):
        self.set_interact_object(triggerIds=[10002031], state=1) # LeverForLadder01
        self.play_system_sound_in_box(sound='System_ShowGuideSummary_01')

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[10002031], stateValue=0):
            return LadderOnToNextMap01(self.ctx)


class LadderOnToNextMap01(common.Trigger):
    def on_enter(self):
