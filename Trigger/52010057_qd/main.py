...
class 연출01(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.check_user():
            ...
            self.set_mesh_animation(trigger_ids=[9002], visible=False, start_delay=0, interval=0)
...
class 연출03(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_time_scale(enable=True, start_scale=0.8, end_scale=0.8, duration=8, interpolator=1) # 2초간 느려지기 시작
...
class 연출04_b(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh_animation(trigger_ids=[9002], visible=True, start_delay=0, interval=0)
