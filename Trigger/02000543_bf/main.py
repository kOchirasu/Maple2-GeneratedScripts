...
class idle(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_effect(trigger_ids=[3005], visible=False)
        self.set_skill(trigger_ids=[4000], enable=False)
        self.set_skill(trigger_ids=[4001], enable=False)
...
class 게임안내2(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.select_camera_path(path_ids=[7000], return_view=True)
        self.lock_my_pc(is_lock=False)
        self.add_balloon_talk(spawn_id=104, msg='$02000543_BF__MAIN__2$', duration=3500, delay_tick=0)
