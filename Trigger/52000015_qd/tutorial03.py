...
class 이슈라대화02대기(common.Trigger):
    def on_enter(self):
...
class 이슈라대화03대기(common.Trigger):
    def on_enter(self):
        self.remove_cinematic_talk()
...
class 이슈라대화04대기CSkip(common.Trigger):
    def on_enter(self):
        self.create_monster(spawnIds=[202])
        self.move_user(mapId=52000015, portalId=50, boxId=9001)
        self.remove_cinematic_talk()
...
class 이슈라대화04대기(common.Trigger):
    def on_enter(self):
        self.remove_cinematic_talk()
...
class 위기상황연출시작01(common.Trigger):
    def on_enter(self):
...
class 위기상황연출시작02(common.Trigger):
    def on_enter(self):
