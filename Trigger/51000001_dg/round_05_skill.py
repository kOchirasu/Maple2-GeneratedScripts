...
class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[3501], visible=False, start_delay=0, interval=0, fade=0)
        self.set_mesh(trigger_ids=[3502], visible=False, start_delay=0, interval=0, fade=0)
        self.set_mesh(trigger_ids=[3503], visible=False, start_delay=0, interval=0, fade=0)
...
class 지역랜덤(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.random_condition(weight=33):
            self.set_mesh(trigger_ids=[3501], visible=True, start_delay=0, interval=0, fade=0)
            return A지역(self.ctx)
        if self.random_condition(weight=33):
            self.set_mesh(trigger_ids=[3502], visible=True, start_delay=0, interval=0, fade=0)
            return B지역(self.ctx)
        if self.random_condition(weight=34):
            self.set_mesh(trigger_ids=[3503], visible=True, start_delay=0, interval=0, fade=0)
            return C지역(self.ctx)
