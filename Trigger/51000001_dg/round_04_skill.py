...
class 시작대기중(state.State):
    def on_enter(self):
        ...
        set_mesh(triggerIds=[3402], visible=False, arg3=0, arg4=0, arg5=0)
        set_mesh(triggerIds=[3403], visible=False, arg3=0, arg4=0, arg5=0)
        set_mesh(triggerIds=[3404], visible=False, arg3=0, arg4=0, arg5=0)
...
class 지역랜덤(state.State):
    def on_tick(self) -> state.State:
        ...
        if random_condition(rate=25):
            ...
            return B지역()
        if random_condition(rate=25):
            set_mesh(triggerIds=[3403], visible=True, arg3=0, arg4=0, arg5=0)
            return C지역()
        if random_condition(rate=25):
            set_mesh(triggerIds=[3404], visible=True, arg3=0, arg4=0, arg5=0)
            return D지역()
...
class C지역(state.State):
    def on_tick(self) -> state.State:
        if user_detected(boxIds=[10403]):
            return 스킬랜덤()


class D지역(state.State):
    def on_tick(self) -> state.State:
        if user_detected(boxIds=[10404]):
            return 스킬랜덤()
