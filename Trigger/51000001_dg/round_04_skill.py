...
class 시작대기중(trigger_api.Trigger):
    def on_enter(self):
        ...
        self.set_mesh(triggerIds=[3402], visible=False, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[3403], visible=False, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[3404], visible=False, arg3=0, delay=0, scale=0)
...
class 지역랜덤(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.random_condition(rate=25):
            ...
            return B지역(self.ctx)
        if self.random_condition(rate=25):
            self.set_mesh(triggerIds=[3403], visible=True, arg3=0, delay=0, scale=0)
            return C지역(self.ctx)
        if self.random_condition(rate=25):
            self.set_mesh(triggerIds=[3404], visible=True, arg3=0, delay=0, scale=0)
            return D지역(self.ctx)
...
class C지역(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(boxIds=[10403]):
            return 스킬랜덤(self.ctx)


class D지역(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(boxIds=[10404]):
            return 스킬랜덤(self.ctx)
