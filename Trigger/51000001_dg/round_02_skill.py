...
class 시작대기중(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[3201], visible=False, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[3202], visible=False, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[3203], visible=False, arg3=0, delay=0, scale=0)
...
class 지역랜덤(common.Trigger):
    def on_tick(self) -> common.Trigger:
        ...
        if self.random_condition(rate=34):
            self.set_mesh(triggerIds=[3203], visible=True, arg3=0, delay=0, scale=0)
            return C지역(self.ctx)
...
class C지역(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_detected(boxIds=[10203]):
            return 스킬랜덤(self.ctx)
