...
class 시작대기중(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[3301], visible=False, start_delay=0, interval=0, fade=0)
        self.set_mesh(trigger_ids=[3302], visible=False, start_delay=0, interval=0, fade=0)
        self.set_mesh(trigger_ids=[3303], visible=False, start_delay=0, interval=0, fade=0)
...
class 지역랜덤(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.random_condition(weight=34):
            self.set_mesh(trigger_ids=[3303], visible=True, start_delay=0, interval=0, fade=0)
            return C지역(self.ctx)
