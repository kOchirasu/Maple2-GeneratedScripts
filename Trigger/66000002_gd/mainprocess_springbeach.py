...
class 이벤트대기중(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.count_users(boxId=302, minUsers='50'):
...
class 경험치지급(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        # self.give_exp(boxId=301, amount=40.5)
        # self.give_exp(boxId=301, amount=9, arg3='1')
...
class 돈벼락(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_timer(timerId='39', seconds=20)
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.time_expired(timerId='39'):
            return 다리등장(self.ctx)
