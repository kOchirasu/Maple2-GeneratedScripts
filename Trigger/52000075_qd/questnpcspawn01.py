...
# 40002668 퀘스트 완료 상태
class RemoveNpc01(common.Trigger):
    def on_enter(self):
...
class MobChange01(common.Trigger):
    def on_enter(self):
...
class MobChange02(common.Trigger):
    def on_enter(self):
...
class MobChange03(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1000):
            return MobChange04(self.ctx)


class MobChange04(common.Trigger):
    def on_enter(self):
        self.set_onetime_effect(id=1, enable=False, path='BG/Common/ScreenMask/Eff_fadein_1sec.xml')
...
class MobTalk01(common.Trigger):
    def on_enter(self):
        self.set_cinematic_ui(type=1)
        self.set_cinematic_ui(type=3)
...
class MobTalk02(common.Trigger):
    def on_enter(self):
...
class MobTalk03(common.Trigger):
    def on_enter(self):
...
class GuardDown01(common.Trigger):
    def on_enter(self):
        self.destroy_monster(spawnIds=[101,201])
