...
class Wait(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000071], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001161 걸어서 71001061 제거
...
class Setting(common.Trigger):
    ...
    def on_tick(self) -> common.Trigger:
        ...
        if self.object_interacted(interactIds=[12000079], stateValue=0):
            self.set_timer(timerId='10', seconds=90, startDelay=1, interval=0, vOffset=0)
...
class TimeTrial_Success(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000071], state=1) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001161 걸어서 71001061 제거

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[12000071], stateValue=0):
        ...
        if self.time_expired(timerId='100'):
            self.set_interact_object(triggerIds=[12000071], state=0)
