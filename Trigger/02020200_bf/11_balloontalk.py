...
class 대사1(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_detected(boxIds=[921]) and not self.monster_dead(boxIds=[205]):
            return 대사2(self.ctx)
        if self.user_detected(boxIds=[922]) and not self.monster_dead(boxIds=[205]):
            return 대사2(self.ctx)
        if self.user_detected(boxIds=[923]) and not self.monster_dead(boxIds=[205]):
            return 대사2(self.ctx)
