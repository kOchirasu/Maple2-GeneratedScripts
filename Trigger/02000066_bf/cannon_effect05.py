...
class 시작(common.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[805], visible=False)
...
class 이펙트대기(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.npc_detected(boxId=115, spawnIds=[8005]):
            return 이펙트on(self.ctx)


class 이펙트on(common.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[805], visible=True)

    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[8005]):
