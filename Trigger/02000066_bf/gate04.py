...
class 생성(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[10000336], state=0)
...
class 게이트열림(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[10000336], state=1)
...
class 게이트닫힘(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[10000336], stateValue=0):
            return 생성(self.ctx)
