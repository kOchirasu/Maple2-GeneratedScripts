...
class 시작(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10000342], state=0)
...
class 오브젝트반응(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10000342], state=1)

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10000342], arg2=0):
            return 부활()
...
class NPC대사(state.State):
    def on_enter(self):
        set_actor(triggerId=202, visible=False, initialSequence='Dead_A')
        set_conversation(type=1, spawnId=2002, script='$02000066_BF__NPC02__1$', arg4=2)
