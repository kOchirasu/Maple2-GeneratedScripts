...
class 대기(common.Trigger):
    def on_enter(self):
        ...
        self.set_portal(portalId=1, visible=False, enable=False, minimapVisible=False) # mesh.xml 에 있던 내용 2001스폰 아이디의 몬스터가 전투 상태로 되기 전에 죽어버리면 mesh.xml 내용이 작동 안하는 버그가 있어서 mesh.xml  내용을 이 main.xml에 포함시키는 형태로 수정했음
...
class 엘리트스폰(common.Trigger):
    def on_enter(self):
        ...
        self.play_system_sound_in_box(sound='System_ShowGuideSummary_01') # mesh.xml 에 있던 내용 2001스폰 아이디의 몬스터가 전투 상태로 되기 전에 죽어버리면 mesh.xml 내용이 작동 안하는 버그가 있어서 mesh.xml  내용을 이 main.xml에 포함시키는 형태로 수정했음
