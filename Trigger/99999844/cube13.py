...
class 큐브13(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4022,4023,4024], visible=True, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[5022,5023,5024], visible=True, arg3=0, delay=0, scale=0)

    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=5000):
            self.set_user_value(triggerId=920013, key='CubeOff', value=1)
...
class 큐브제거(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4022,4023,4024], visible=False, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[5022,5023,5024], visible=False, arg3=0, delay=0, scale=0)

    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=4200):
            return 큐브13(self.ctx)
        if self.user_detected(boxIds=[9004]):
            self.set_user_value(triggerId=910013, key='Cube', value=0)
            return 종료(self.ctx)
