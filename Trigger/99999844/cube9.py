...
class 큐브9(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4014,4015,4016,4017], visible=True, arg3=0, delay=0, scale=0)
        self.set_mesh(triggerIds=[5014,5015,5016,5017], visible=True, arg3=0, delay=0, scale=0)

    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=5000):
            self.set_user_value(triggerId=920009, key='CubeOff', value=1)
...
class 큐브제거(common.Trigger):
    def on_enter(self):
        self.set_mesh(triggerIds=[4014,4015,4016,4017], visible=False, arg3=0, delay=0, scale=0)
    ...
    def on_tick(self) -> common.Trigger:
        ...
        if self.user_detected(boxIds=[9004]):
            self.set_user_value(triggerId=910009, key='Cube', value=0)
            return 종료(self.ctx)
