...
# 첫 번째 퀘스트 완료 상태
class QuestOnGoing10(common.Trigger):
    def on_tick(self) -> common.Trigger:
...
# 두 번째 퀘스트 수락한 상태
class QuestOnGoing20(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=300):
...
# 세 번째 퀘스트 수락한 상태
class QuestOnGoing30(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=300):
...
class MoveToFindStone01(common.Trigger):
    def on_enter(self):
...
class MoveToFindStone02(common.Trigger):
    def on_enter(self):
        self.set_effect(triggerIds=[5002], visible=True) # 미션 완료 사운드 이펙트
...
class ThirdQuestStart01(common.Trigger):
    def on_enter(self):
        self.show_guide_summary(entityId=10030160, textId=10030160) # 가이드 : [[icon:questaccept]] 틴차이와 대화하기
        self.set_effect(triggerIds=[5000], visible=True) # 가이드 서머리 사운드 이펙트
