...
class Setting(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001153], state=0) # Lever_BlockStart
        set_interact_object(triggerIds=[10001154], state=0) # Lever_BlockOff
...
class BlockEnable(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001153], state=1) # Lever_BlockStart

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001153], arg2=0):
            return BlockStart()
...
class BlockDisable(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001154], state=1) # Lever_BlockOff

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001154], arg2=0):
