...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001148], state=0) # Chair
...
class LoadingDelay(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001148], state=1) # Chair
...
class MobTrapOn(state.State):
    ...
    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001148], arg2=0):
            return RockMove01()
