...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001144], state=0) # Document
...
class True(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001144], state=1) # Document

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001144], arg2=0):
            return KeyFound()
