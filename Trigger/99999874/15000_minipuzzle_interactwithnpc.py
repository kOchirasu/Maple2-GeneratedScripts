...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[12000070], state=2) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001151 걸어서 71001051 제거
...
class InteractWithNpc_NpcTypeA_StandAside(state.State):
    ...
    def on_tick(self) -> state.State:
        if true():
            set_effect(triggerIds=[15303], visible=False)
...
class InteractWithNpc_Success(state.State):
    def on_enter(self):
        ...
        set_actor(triggerId=15100, visible=False, initialSequence='Idle_A') # Dummy RareBox
        set_interact_object(triggerIds=[12000070], state=1) # RareBox / 기믹 종료 오브젝트 / Additional Effect 71001151 걸어서 71001051 제거

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[12000070], arg2=0):
