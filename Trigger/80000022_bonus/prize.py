...
class 입장(state.State):
    def on_enter(self):
        ...
        set_actor(triggerId=3000, visible=False, initialSequence='or_functobj_mortar_A01_off') # 오브젝트 반응 이후 사라진 달토끼 대체인력 끈 상태로 정의
        set_interact_object(triggerIds=[11000119], state=1) # 달토끼 오브젝트, 반응 가능으로 스폰
...
class 오브젝트체크(state.State):
    ...
    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[11000119], arg2=0):
            return 포탈생성()


class 포탈생성(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[11000119], state=2) # 달토끼 오브젝트, 사라짐
        set_actor(triggerId=3000, visible=True, initialSequence='or_functobj_mortar_A01_off') # 오브젝트 반응 이후 사라진 달토끼 대체인력 켜기
