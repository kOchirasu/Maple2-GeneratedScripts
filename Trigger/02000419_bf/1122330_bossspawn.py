...
class 몬스터등장(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(triggerIds=[2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015], visible=False, arg3=0, delay=200, scale=0.5)
        ...
        self.set_mesh(triggerIds=[1001,1002,1003,1004,1005,1006,1007,1008,1009,1010,1011,1012,1013,1014,1015,1016,1017,1018,1019,1020,1021,1022,1023,1024,1025], visible=False, arg3=0, delay=200, scale=0.5)
...
class 두번째전투판이동다리생성(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_mesh(triggerIds=[2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015], visible=True, arg3=1, delay=120, scale=0.5)
        # 두번째 전투판으로 이동하기 위한 다리가 생성되면 5001 : 투명 벽 제거하기, 5002 : 지란트 소환몹이 등장하는 곳에 설치된 투명벽도 제거하기
        self.set_mesh(triggerIds=[5001,5002], visible=False, arg3=0, delay=0, scale=0)
