...
class idle(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_effect(trigger_ids=[7007], visible=False)
...
class start(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.select_camera_path(path_ids=[604,603], return_view=True)
        self.set_onetime_effect(id=1, enable=False, path='BG/Common/ScreenMask/Eff_fadein_1sec.xml')
        self.show_caption(type='VerticalCaption', title='$02000532_BF__MAIN__0$', desc='$02000532_BF__MAIN__1$', align='centerRight', offset_rate_x=0, offset_rate_y=0, duration=3000, scale=2)
...
class 문들어가기(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(spawn_ids=[408], arg2=False):
            return 문이열림(self.ctx)


class 문이열림(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_mesh(trigger_ids=[3004], visible=False)
        self.add_balloon_talk(spawn_id=112, msg='$02000532_BF__MAIN__4$', duration=3500, delay_tick=0)
        ...
        self.set_npc_emotion_loop(spawn_id=102, sequence_name='Talk_A', duration=10000)
        self.set_npc_emotion_loop(spawn_id=113, sequence_name='Sit_Down_A', duration=10000)
...
class 경계하기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_balloon_talk(spawn_id=104, msg='$02000532_BF__MAIN__8$', duration=3500, delay_tick=0)
...
class 흑성회의반격(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_cinematic_ui(type=0)
        self.set_cinematic_ui(type=2)
        self.set_mesh(trigger_ids=[3002,3003], visible=False)
        self.lock_my_pc(is_lock=False)
        self.reset_camera(interpolation_time=1)
...
class 흑성회의반격2(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.spawn_monster(spawn_ids=[401,402,403,404,412,405], auto_target=True)
...
class 길목에서나오는몬스터(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.spawn_monster(spawn_ids=[406,407,409,410,411], auto_target=True)
