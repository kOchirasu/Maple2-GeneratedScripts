...
class 문열기오브젝트설정1(common.Trigger):
    def on_enter(self):
...
class 문을열자1(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.monster_dead(boxIds=[101]):
            return 감옥문부시기2(self.ctx)


class 감옥문부시기2(common.Trigger):
    def on_enter(self):
...
class 문열기오브젝트설정2(common.Trigger):
    def on_enter(self):
...
class 몬스터다수생성하기(common.Trigger):
    def on_enter(self):
