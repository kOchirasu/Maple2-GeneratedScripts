...
class Wait_Talk01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_timer(timerId='1', seconds=90, startDelay=1, interval=1, vOffset=-80)
...
class CheckTheNumberOfPlayers_1st(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.count_users(boxId=9000, minUsers='25', operator='GreaterEqual'):
            # 25명 이상이면 게임 시작
            return MatchingSuccessDelay(self.ctx)
        if self.count_users(boxId=9000, minUsers='25', operator='Less'):
            # 25명 미만이면 게임 취소
