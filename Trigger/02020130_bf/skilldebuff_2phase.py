...
class Ready(state.State):
    def on_enter(self):
        set_user_value(key='SkillDebuffCheck_2Phase', value=0) # SkillDebuffCheck_2Phase 변수 0으로 초기 세팅, 보스가 저주디버프스킬 사용할 때 이 트리거에거 이 변수 1 신호를 보냄
...
class 보스의저주디버프사용신호대기(state.State):
    def on_tick(self) -> state.State:
        if user_value(key='SkillDebuffCheck_2Phase', value=1):
...
class 폭발저주디버프제거잠시대기(state.State):
    def on_enter(self):
        set_user_value(key='SkillDebuffCheck_2Phase', value=0) # 중요:  여기 단계 끝나면 다시 처음인 <state name="보스의저주디버프사용신호대기"> 단계로 넘어가는데, 거기서의  SkillDebuffCheck_2Phase =  1 진행되지 않도록 이 변수 꼭 0 초기화 하기
