...
class 대기(state.State):
    def on_enter(self):
        set_skill(triggerIds=[1100], isEnable=False)
        set_effect(triggerIds=[1101], visible=False) # RockDrop
...
class 낙하01시작(state.State):
    def on_enter(self):
        set_effect(triggerIds=[1101], visible=True) # RockDrop
...
class 낙하01완료(state.State):
    def on_enter(self):
        ...
        set_skill(triggerIds=[1100], isEnable=True)
