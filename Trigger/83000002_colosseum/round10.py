...
class 라운드조건체크(state.State):
    def on_tick(self) -> state.State:
        if dungeon_round_require(round=10):
            side_npc_talk(type='talk', npcId=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND10__0$', duration=5000)
            return 라운드대기()
        if true():
            side_npc_talk(type='talk', npcId=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND10__1$', duration=3000)
...
class 카운트(state.State):
    def on_enter(self):
        show_count_ui(text='$83000002_COLOSSEUM__ROUND10__2$', count=3, soundType=2)
...
class 스폰대사(state.State):
    def on_enter(self):
        add_balloon_talk(spawnId=110, msg='$83000002_COLOSSEUM__ROUND10__3$', duration=3000)
    ...
    def on_tick(self) -> state.State:
        if monster_dead(boxIds=[110]):
            add_balloon_talk(spawnId=110, msg='$83000002_COLOSSEUM__ROUND10__4$', duration=3000)
        ...
        if time_expired(timerId='LimitTimer'):
            side_npc_talk(type='talk', npcId=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND10__5$', duration=3000)
        ...
        if user_detected(boxIds=[902]):
            side_npc_talk(type='talk', npcId=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND10__6$', duration=3000)
            destroy_monster(spawnIds=[110])
            set_npc_duel_hp_bar(isOpen=False, spawnId=[110])
            destroy_monster(spawnIds=[10000])
            destroy_monster(spawnIds=[10001])
        ...
        if not user_detected(boxIds=[904]):
            side_npc_talk(type='talk', npcId=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND10__7$', duration=3000)
            destroy_monster(spawnIds=[110])
            set_npc_duel_hp_bar(isOpen=False, spawnId=[110])
            destroy_monster(spawnIds=[10000])
            destroy_monster(spawnIds=[10001])
...
class ClearRoundDelay(state.State):
    ...
    def on_tick(self) -> state.State:
        if wait_tick(waitTick=2000):
            ...
            set_event_ui(type=3, arg2='$83000002_COLOSSEUM__ROUND10__8$', arg3='3000')
            return ClearRound()
...
class FailRoundDelay(state.State):
    ...
    def on_tick(self) -> state.State:
        if wait_tick(waitTick=3000):
            set_event_ui(type=5, arg2='$83000002_COLOSSEUM__ROUND10__9$', arg3='3000')
            return FailRound()
...
class ClearRound(state.State):
    def on_tick(self) -> state.State:
        if wait_tick(waitTick=3000):
            side_npc_talk(type='talk', npcId=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND10__10$', duration=3000)
