...
class 라운드조건체크(trigger_api.Trigger):
    def on_tick(self) -> trigger_api.Trigger:
        if self.dungeon_round_require(round=6):
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND6__0$', duration=5000)
            # self.set_event_ui(type=1, arg2='승리하셨습니다. 다음 전투에서 승리한 더 강한 도전자들을 만납니다. 긴장하세요.', arg3='3000')
            return 라운드대기(self.ctx)
        self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND6__1$', duration=3000)
...
class 몬스터스폰(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_buff(box_ids=[106], skill_id=69000501, level=1, is_player=True)
...
class 스폰대사(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.monster_dead(spawn_ids=[106]):
            ...
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[106])
        ...
        if self.time_expired(timer_id='LimitTimer'):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND6__5$', duration=3000)
        ...
        if self.user_detected(box_ids=[902]):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND6__6$', duration=3000)
            self.destroy_monster(spawn_ids=[106])
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[106])
            return FailRoundDelay(self.ctx)
        if not self.user_detected(box_ids=[904]):
            # self.set_event_ui(type=1, arg2='패배했습니다. 전투가 종료됩니다.', arg3='3000')
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_switchon', script='$83000002_COLOSSEUM__ROUND6__7$', duration=3000)
            self.destroy_monster(spawn_ids=[106])
            self.set_npc_duel_hp_bar(is_open=False, spawn_id=[106])
            return FailRoundDelay(self.ctx)
...
class ClearRound(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=3000):
            ...
            self.side_npc_talk(type='talk', npc_id=11004288, illust='nagi_normal', script='$83000002_COLOSSEUM__ROUND6__10$', duration=3000)
