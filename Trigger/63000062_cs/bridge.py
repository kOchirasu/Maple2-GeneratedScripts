...
class 대기(common.Trigger):
    def on_tick(self) -> common.Trigger:
        ...
        if self.day_of_week(dayOfWeeks=[3], desc='1(일)-7(토)'):
            ...
            self.set_mesh(triggerIds=[4030], visible=True)
            return 화요일(self.ctx)
        if self.day_of_week(dayOfWeeks=[4], desc='수요일'):
