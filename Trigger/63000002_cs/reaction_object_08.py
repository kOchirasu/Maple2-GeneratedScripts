...
class 채집가능(common.Trigger):
    def on_enter(self):
        self.set_interact_object(triggerIds=[608], state=1)

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[608], stateValue=2):
            self.create_item(spawnIds=[1008])
            return 채집완료(self.ctx)


class 채집완료(common.Trigger):
    def on_enter(self):
        self.set_timer(timerId='8', seconds=30, startDelay=0)
