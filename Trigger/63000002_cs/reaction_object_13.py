...
class 채집가능(trigger_api.Trigger):
    def on_enter(self):
        self.set_interact_object(triggerIds=[613], state=1)

    def on_tick(self) -> trigger_api.Trigger:
        if self.object_interacted(interactIds=[613], stateValue=2):
            self.create_item(spawnIds=[1013])
            return 채집완료(self.ctx)


class 채집완료(trigger_api.Trigger):
    def on_enter(self):
        self.set_timer(timerId='13', seconds=30, startDelay=0)
