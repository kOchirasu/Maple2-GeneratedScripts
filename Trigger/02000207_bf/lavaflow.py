...
class 왼쪽용암생성(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_user_value(triggerId=999102, key='LavaflowLeft', value=0)
        # ### 올라온 용암 내려가게 할 때 사용하는 LavaflowLeftStop 변수 이전에 이미 사용했을 수 있으므로 여기서  0으로 꼭 초기화 해야함
        self.set_user_value(triggerId=999102, key='LavaflowLeftStop', value=0)
        self.set_effect(triggerIds=[600], visible=True)
    ...
    def on_tick(self) -> trigger_api.Trigger:
        ...
        if self.user_value(key='LavaflowLeftStop', value=1):
...
class 오른쪽용암생성(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_user_value(triggerId=999102, key='LavaflowRight', value=0)
        # ### 올라온 용암 내려가게 할 때 사용하는 LavaflowRightStop 변수 이전에 이미 사용했을 수 있으므로 여기서  0으로 꼭 초기화 해야함
        self.set_user_value(triggerId=999102, key='LavaflowRightStop', value=0)
        self.set_effect(triggerIds=[600], visible=True)
    ...
    def on_tick(self) -> trigger_api.Trigger:
        # 보스가 죽으면 보스 스폰시키는 트리거 xml 에서 BattleEnd2 = 1 신호를 보내서 올라와 있는 용암을 여기서 바로 제거 시키게 함
        if self.user_value(key='LavaflowRightStop', value=1):
