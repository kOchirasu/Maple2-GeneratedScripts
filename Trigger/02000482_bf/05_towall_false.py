...
class Wait(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002032], state=0) # ToWall_False
...
class ToWallFalse(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002032], state=1) # ToWall_False

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10002032], arg2=0):
            return NoticeDelay()
