...
class Setting(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10002028], state=0, arg4=False) # LeverForTrap
...
class LeverOn(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002028], state=1) # LeverForTrap
    ...
    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10002028], arg2=0):
            return TrapOn()
