...
class Wait(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002034], state=0) # ToTower_False
...
class ToTowerFalse(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002034], state=1) # ToTower_False

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10002034], arg2=0):
            return NoticeDelay()
