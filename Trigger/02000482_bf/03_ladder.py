...
class Setting(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10002026], state=0, arg4=False) # LeverForLadder
...
class LeverOn(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10002026], state=1) # LeverForLadder

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10002026], arg2=0):
            return LadderOn()
