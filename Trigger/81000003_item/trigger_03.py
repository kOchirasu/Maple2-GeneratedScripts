...
class 대기(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        """
        if self.count_users(box_id=402, min_users='20'):
...
class 시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_timer(timer_id='160', seconds=160, start_delay=0, interval=1, desc='trigger_03.xml 시작 타이머 설정')
...
class 경기종료(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(wait_tick=6000):
            self.end_mini_game_round(winner_box_id=401, exp_rate=0.13333334, is_gain_loser_bonus=True, game_name='UserMassive_Crazyrunner') # 승리자 및 실패자도 경험치 지급함
