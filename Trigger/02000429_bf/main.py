...
class 인페르녹처치성공(state.State):
    def on_enter(self):
        set_achievement(triggerId=750, type='trigger', achieve='infernogout') # arg3="infernogout" 는 퀘스트 완료 조건 처리 키값임
...
class 최종성공처리(state.State):
    def on_enter(self):
        ...
        show_guide_summary(entityId=20041012, textId=20041012) # 인페르녹 몰아내기 성공 알림 메시지
    ...
    def on_exit(self):
        hide_guide_summary(entityId=20041012) # 인페르녹 몰아내기 성공 알림 메시지
