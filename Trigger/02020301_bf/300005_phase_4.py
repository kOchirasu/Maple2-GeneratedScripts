...
class 패이즈_4_시작(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.side_npc_talk(type='talk', npcId=11004205, illust='ArcaneBlader_unfair', script='$02020301_BF__300005_PHASE_4__0$', duration=3176)
...
class 추가대화(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.side_npc_talk(type='talk', npcId=29500101, illust='ArcheonBlack_Angry', script='$02020301_BF__300005_PHASE_4__1$', duration=3176)
...
class 추가대화_2(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.side_npc_talk(type='talk', npcId=11004205, illust='ArcaneBlader_unfair', script='$02020301_BF__300005_PHASE_4__2$', duration=3176)
...
class 엘리베이터_대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.add_buff(boxIds=[101], skillId=62100108, level=1, isPlayer=True) # 스택버프 걸어주기
        self.side_npc_talk(type='talk', npcId=11004205, illust='ArcaneBlader_unfair', script='$02020301_BF__300005_PHASE_4__3$', duration=3176)
        self.set_user_value(key='AI_Phase', value=0)
        # 좌우 엘리베이터 숨기기(페이즈가 바뀜으로써..)추후 파괴하는걸로 연출을 변경해보자.
        self.set_visible_breakable_object(triggerIds=[5351,5352,5353,5354,5355,5356,5357,5358,5359,5360,5361,5362,5363,5364], visible=False)
...
class 폭발물제어장치_생성(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.create_monster(spawnIds=[111], animationEffect=True) # 아르케온 한방기 폭발 ai
