...
class 시작대기중(state.State):
    def on_enter(self):
        ...
        set_portal(portalId=598, visible=False, enabled=False, minimapVisible=False) # 세번째페이즈 진행하는 맵 이동 포탈 처음에 감추기, 최초 시작 지점에 설치한 포탈
    ...
    def on_tick(self) -> state.State:
        if quest_user_detected(boxIds=[99], questIds=[10003330], questStates=[2]):
            return 이동()
        if quest_user_detected(boxIds=[99], questIds=[10003330], questStates=[3]):
            return 이동()
...
class 졸구간시작연출딜레이(state.State):
    def on_enter(self):
        ...
        change_background(dds='BG_Turka_A.dds') # 2페이즈 부터 맵이 밝아지면서, 백그라운드 이미지 교체함
...
class 최초졸구간몬스터등장(state.State):
    def on_enter(self):
        ...
        create_monster(spawnIds=[1201,1202,1203,1204], arg2=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 졸구간진행체크중(state.State):
    def on_tick(self) -> state.State:
        ...
        if user_value(key='2PhaseStartOk', value=1):
            return 모든졸구간지형과포탈생성()
        if user_value(key='TurkaTwoPhaseEnd', value=1): # ## 퀘스트용 투르카 던전은 가운데 지형만 지형하고 왼쪽 오른쪽 지형은 진행 필요 없어 주석처리함 ##
...
class 두번째왼쪽진행01(state.State):
    def on_enter(self):
        ...
        create_monster(spawnIds=[2101,2102,2103,2104,2105,2106,2107,2108,2109,2110,2111,2112,2113,2114,2115,2116,2117,2118], arg2=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 두번째가운데진행01(state.State):
    def on_enter(self):
        ...
        create_monster(spawnIds=[2201,2202,2203,2204], arg2=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 세번째가운데진행01(state.State):
    def on_enter(self):
        ...
        create_monster(spawnIds=[3201,3202,3203,3204], arg2=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 두번째오른쪽진행01(state.State):
    def on_enter(self):
        ...
        create_monster(spawnIds=[2301,2302,2303,2304,2305,2306,2307,2308,2309,2310,2311,2312,2313,2314,2315,2316,2317,2318], arg2=False) # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
...
class 모든졸구간지형과포탈생성(state.State):
    def on_enter(self):
        ...
        set_portal(portalId=428, visible=True, enabled=True, minimapVisible=True) # 보스 전투판으로 가는 가운데 졸구간 포탈 생성
        set_portal(portalId=338, visible=True, enabled=True, minimapVisible=True) # 보스 전투판으로 가는 오른쪽 졸구간 포탈 생성
        destroy_monster(spawnIds=[1101,1102,1103,1104,1105,1106,1107,1108,1109,1110,1111,1112,1113,1114,2101,2102,2103,2104,2105,2106,2107,2108,2109,2110,2111,2112,2113,2114,2115,2116,2117,2118])
        destroy_monster(spawnIds=[1201,1202,1203,1204,2201,2202,2203,2204,3201,3202,3203,3204])
        destroy_monster(spawnIds=[1301,1302,1303,1304,1305,1306,1307,1308,1309,1310,1311,1312,1313,2301,2302,2303,2304,2305,2306,2307,2308,2309,2310,2311,2312,2313,2314,2315,2316,2317,2318])
...
class 이맵에서진행끝내고다음맵으로이동(state.State):
    def on_enter(self):
        ...
        set_portal(portalId=598, visible=True, enabled=True, minimapVisible=True) # 세번째페이즈 진행하는 다음 맵으로 이동시키기, 최초 시작 지점에 위치한 포탈
