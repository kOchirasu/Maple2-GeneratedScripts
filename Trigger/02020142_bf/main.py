...
class 시작대기중(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.quest_user_detected(box_ids=[99], quest_ids=[10003330], quest_states=[2]):
            return 이동(self.ctx)
        if self.quest_user_detected(box_ids=[99], quest_ids=[10003330], quest_states=[3]):
            return 이동(self.ctx)
...
class 최초졸구간몬스터등장(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.spawn_monster(spawn_ids=[1201,1202,1203,1204], auto_target=False)
...
class 두번째왼쪽진행01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
        self.spawn_monster(spawn_ids=[2101,2102,2103,2104,2105,2106,2107,2108,2109,2110,2111,2112,2113,2114,2115,2116,2117,2118], auto_target=False)
...
class 두번째가운데진행01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
        self.spawn_monster(spawn_ids=[2201,2202,2203,2204], auto_target=False)
...
class 세번째가운데진행01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
        self.spawn_monster(spawn_ids=[3201,3202,3203,3204], auto_target=False)
...
class 두번째오른쪽진행01(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        # 중요: 이 졸몹은 등장하자마자 AI에서 트리거 쪽으로 TriggerMesh?? 변수 +1 신호를 보내야 하기 때문에 꼭 등장하자마자 바로 전투 상태가 되어야 함
        self.spawn_monster(spawn_ids=[2301,2302,2303,2304,2305,2306,2307,2308,2309,2310,2311,2312,2313,2314,2315,2316,2317,2318], auto_target=False)
...
class 모든졸구간지형과포탈생성(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_portal(portal_id=338, visible=True, enable=True, minimap_visible=True)
        # 그리고 졸 몬스터 전부 제거하기
        self.destroy_monster(spawn_ids=[1101,1102,1103,1104,1105,1106,1107,1108,1109,1110,1111,1112,1113,1114,2101,2102,2103,2104,2105,2106,2107,2108,2109,2110,2111,2112,2113,2114,2115,2116,2117,2118])
        self.destroy_monster(spawn_ids=[1201,1202,1203,1204,2201,2202,2203,2204,3201,3202,3203,3204])
        self.destroy_monster(spawn_ids=[1301,1302,1303,1304,1305,1306,1307,1308,1309,1310,1311,1312,1313,2301,2302,2303,2304,2305,2306,2307,2308,2309,2310,2311,2312,2313,2314,2315,2316,2317,2318])
