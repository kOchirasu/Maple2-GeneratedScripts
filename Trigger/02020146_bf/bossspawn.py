...
class 기본셋팅(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.set_portal(portalId=601, visible=False, enable=False, minimapVisible=False)
...
class 전투진행(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        # 맵 나가기 포탈, 전투판으로 가기 위한 맵 내부 포탈
        self.set_portal(portalId=601, visible=True, enable=True, minimapVisible=True)
...
class 마무리연출(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.wait_tick(waitTick=9000):
            self.set_portal(portalId=1, visible=True, enable=True, minimapVisible=True) # 맵 나가기 포탈, 시작 지점에
            self.set_portal(portalId=2, visible=True, enable=True, minimapVisible=True) # 맵 나가기 포탈, 전투판 지점에
            return 종료(self.ctx)
