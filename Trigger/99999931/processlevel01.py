...
class 레버당기기(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10000217], state=1)

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10000217], arg2=0):
            return 카운트다운1()
...
class 게임시작(state.State):
    def on_enter(self):
        ...
        set_breakable(triggerIds=[101,102,103,104,105,106,107,114,115,116,118,119,121,123,126,130,131,132,133,134,135], enabled=True) # 움직이는 발판을 이동한다 (arg2=1)
...
class 게임진행1(state.State):
    def on_enter(self):
        ...
        set_breakable(triggerIds=[101,102,103,104,105,106,107,114,115,116,118,119,121,123,126,130,131,132,133,134,135], enabled=False) # 움직이는 발판을 멈춘다 (arg2=0)
        set_skill(triggerIds=[801,802,803,804,805,806,807], isEnable=True)
