...
class Wait(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000200], state=2) # RareBox 맵 별로 유니크하도록 변경해야 하는 값
        self.set_interact_object(triggerIds=[12000074], state=2) # AntiqueMap / 기믹 시작 오브젝트 / Additional Effect 71001011 부여
...
class EndMelodyQuiz_Success(common.Trigger):
    def on_enter(self):
        ...
        self.set_interact_object(triggerIds=[12000200], state=1) # RareBox 맵 별로 유니크하도록 변경해야 하는 값
        self.set_timer(timerId='2', seconds=60, startDelay=1, interval=0, vOffset=0)

    def on_tick(self) -> common.Trigger:
        if self.object_interacted(interactIds=[12000200], stateValue=0):
