...
class MelodyPlay02_Start(state.State):
    def on_enter(self):
        set_effect(triggerIds=[11206], visible=True) # Bell D
        set_actor(triggerId=11006, visible=True, initialSequence='ks_quest_musical_B01_navy') # Bell D
...
class MelodyPlay02_End(state.State):
    def on_enter(self):
        set_actor(triggerId=11006, visible=True, initialSequence='ks_quest_musical_B01_off') # Bell D
...
class CheckAnswer02_Delay(state.State):
    ...
    def on_exit(self):
        set_interact_object(triggerIds=[12000063], state=1) # Lever D
