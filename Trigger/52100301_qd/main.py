...
class 대기(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10003126], state=2) # 2페이즈 인터렉트 오브젝트 대기
...
class 시작(state.State):
    def on_enter(self):
        ...
        side_npc_talk(type='talk', npcId=29500101, illust='ArcheonBlack_Normal', script='$52100301_QD__MAIN__0$', duration=5684)
...
class 보스전_성공(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10003126], state=2) # 2페이즈 인터렉트 오브젝트 대기
        set_user_value(triggerId=3000061, key='Phase_5_Interect_01', value=0)
        add_buff(boxIds=[1003], skillId=62100169, level=1)
        dungeon_mission_complete(missionId=23039005) # 포탑 기절 이뮨 제거
        dungeon_set_end_time()
        side_npc_talk(type='talk', npcId=29500101, illust='ArcheonBlack_Die', script='$52100301_QD__MAIN__1$', duration=3176)
        set_achievement(type='trigger', achieve='KillArcheon')
...
class 추가대화(state.State):
    def on_enter(self):
        side_npc_talk(type='talk', npcId=11004205, illust='ArcaneBlader_normal', script='$52100301_QD__MAIN__2$', duration=3176)
