...
class 유저이동_2(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1000):
            return 조명리셋_2(self.ctx)


class 조명리셋_2(common.Trigger):
    def on_enter(self):
...
class 중앙지역이동_2_페이드인(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=100):
            return 포탈설정_2(self.ctx)


class 포탈설정_2(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=5, visible=True, enable=True, minimapVisible=True)
