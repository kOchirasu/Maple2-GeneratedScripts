...
# **********************환경 변화 페이즈_3************************************************************************************************
class 환경변화_3(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='Movement', value=0):
            return 시작(self.ctx)
        if self.user_value(key='dark', value=5):
            return 페이드아웃_3(self.ctx)


class 페이드아웃_3(common.Trigger):
    def on_enter(self):
...
class 유저이동_3(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1000):
            return 조명리셋_3(self.ctx)


class 조명리셋_3(common.Trigger):
    def on_enter(self):
        self.set_conversation(type=1, spawnId=101, script='$02020111_BF__MOVEMENT_01__2$', arg4=3, arg5=0)
        self.move_npc_to_pos(spawnId=101, pos=[-8,-3318,1651], rot=[0,0,45])
        self.set_onetime_effect(id=1, enable=False, path='BG/Common/ScreenMask/Eff_fadein_1sec.xml')
        ...
        self.set_effect(triggerIds=[200021,200022,200023,200024,200025], visible=True)
    ...
    def on_tick(self) -> common.Trigger:
        if self.user_value(key='Movement', value=0):
            return 시작(self.ctx)
        if self.user_value(key='dark', value=6):
            return 중앙지역이동_3(self.ctx)


class 중앙지역이동_3(common.Trigger):
    def on_enter(self):
...
class 중앙지역이동_3_페이드인(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=100):
            return 포탈설정_3(self.ctx)


class 포탈설정_3(common.Trigger):
    def on_enter(self):
        self.set_portal(portalId=5, visible=True, enable=True, minimapVisible=True)
        self.set_portal(portalId=6, visible=True, enable=True, minimapVisible=True)
