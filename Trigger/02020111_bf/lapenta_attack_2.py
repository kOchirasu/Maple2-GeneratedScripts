...
class 시작(trigger_api.Trigger):
    ...
    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='Lapenta_Attack_2', value=1):
            return 대기(self.ctx)


class 대기(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.side_npc_talk(type='talk', npc_id=23501011, illust='Turned_Renduebian_normal', script='$02020111_BF__LAPENTA_ATTACK_2__0$', duration=3525, voice='ko/Npc/00002200')
...
class 스킬발동(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        self.set_effect(trigger_ids=[200001,200002,200003,200004,200005,200011,200012,200013,200014,200015,200021,200022,200023,200024,200025,200031,200032,200033,200034,200035], visible=False)
...
class 실패조건버프(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.add_buff(box_ids=[101], skill_id=70002181, level=1, is_player=True)

    def on_tick(self) -> trigger_api.Trigger:
        if self.user_value(key='Lapenta_Attack_2', value=0):
            return 시작(self.ctx)
