...
class 시작(state.State):
    ...
    def on_tick(self) -> state.State:
        if user_value(key='Lapenta_Attack', value=1):
            return 대기()


class 대기(state.State):
    def on_enter(self):
        side_npc_talk(type='talk', npcId=23501011, illust='Turned_Renduebian_normal', script='$02020111_BF__LAPENTA_ATTACK__0$', duration=3432, voice='ko/Npc/00002199')
        set_ambient_light(primary=[52,48,38])
        set_directional_light(diffuseColor=[0,0,0], specularColor=[206,174,84])

    def on_tick(self) -> state.State:
        if wait_tick(waitTick=3432):
            return 스킬발동()
...
class 실패조건버프(state.State):
    ...
    def on_tick(self) -> state.State:
        if user_value(key='Lapenta_Attack', value=0):
            return 시작()
