...
class Wait(state.State):
    def on_enter(self):
        ...
        set_interact_object(triggerIds=[10001247], state=2)
...
class MachineOn(state.State):
    def on_enter(self):
        set_interact_object(triggerIds=[10001247], state=1)

    def on_tick(self) -> state.State:
        if object_interacted(interactIds=[10001247], arg2=0):
            return PortalOn()


class PortalOn(state.State):
    def on_enter(self):
        set_portal(portalId=4, visible=False, enabled=True, minimapVisible=False)
