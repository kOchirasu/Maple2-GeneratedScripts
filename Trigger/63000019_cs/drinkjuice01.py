...
# 첫 번째 퀘스트 완료 가능 상태
class MoveToWeiHong01(common.Trigger):
    def on_enter(self):
...
class TalkToWeiHong01(common.Trigger):
    def on_enter(self):
...
class TalkToWeiHong02(common.Trigger):
    def on_enter(self):
...
class MoveToBartender01(common.Trigger):
    def on_enter(self):
...
class MoveToBartender02(common.Trigger):
    def on_enter(self):
...
class TalkToBartender01(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.quest_user_detected(boxIds=[9900], questIds=[90000438], questStates=[3]):
            return QuestComplete01(self.ctx)
...
# 두번째 퀘스트 및 가이드 완료 상태
class TalkingDelay01(common.Trigger):
    def on_tick(self) -> common.Trigger:
        if self.wait_tick(waitTick=1000):
