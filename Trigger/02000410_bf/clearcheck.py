...
class 분완료15(trigger_api.Trigger):
    def on_enter(self) -> 'trigger_api.Trigger':
        ...
        self.dungeon_mission_complete(feature='DungeonRankBalance_01', missionId=24090003)
        # ## 중국용 던전랭크 코드: 15분 경과시 던전랭크 체크하기 위한 신호
        self.dungeon_mission_complete(feature='DungeonRankBalance_02', missionId=24090013)

    def on_tick(self) -> trigger_api.Trigger:
        # 인페르녹 보스 스폰ID : 102 의 몬스터가 지금까지 받은 대미지가 HP 기준 대비 100%보다 적으면 던전 실패 처리
        if self.check_npc_damage(spawnId=102, damageRate=1, operator='GreaterEqual'):
